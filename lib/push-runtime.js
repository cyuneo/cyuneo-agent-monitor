...
//   event is dropped without claiming it, so it is never sent late.
...
function createPushRuntime(deps) {
  ...
  /** Quiet hours hold this event type now (a failing callback mutes nothing) */
  function muted(type) {
  ...
  /**
   * After each render.
   * @param {{ sessions: any[], lamps: any, quota: any, now?: number, seq?: number, alerts?: any[] }} input seq: count of
   *   real snapshots (replays not counted); a waiting needsYou check goes ahead once it grows past the count at its rescan
   *   request. alerts: threshold alerts from lib/alerts.js createThresholdTracker().update on the same data
   * @returns {any[]} the tracker's events and the alerts passed in (for tests and logs)
   */
  ...
  async function enqueue(ev, p) {
    if (!ev || muted(ev.type)) return; // quiet hours: dropped, not claimed (another window outside them may send it)

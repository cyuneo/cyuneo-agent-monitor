...
//   The extension also hands over the threshold alerts of lib/alerts.js (usageHigh / costDaily / contextHigh) it found on
//   the same data; they go out at once, like errors and limits.
// - Auto-resume (DESIGN §12): update() also takes the plans (autoResume: Map<session key, Plan>, from the auto-resume
//   runtime updated on the same snapshot) and copies a session's plan onto an error / limitHit event when the event is
//   created (push.autoResumeSnapshot), so the message tells the plan as it was at the stop. The outcomes it is handed
//   (autoResumeEvents: Outcome[]) become 'autoResume' events and go out at once, claimed once across windows. An
//   update() with outcomes only (no sessions) leaves the tracker and the pending waits as they are.
// - Quiet hours (deps.mute, lib/alerts.js shouldMute('push', …)): checked when an event is about to be queued; a muted
//   event is dropped without claiming it, so it is never sent late. A failed or given-up auto-resume is asked about as an
//   'error' (so allowErrors lets it through); the other outcomes as 'autoResume'.
// - Push off, the network switch off (agentMonitor.network.allow, read through lib/network.js), or no channel in
//   settings: nothing is planned, claimed, queued or sent. No timer, no network. The tracker still runs, so allowing the
//   network later never sends what happened while it was off. Every send goes through network.request().
 ...
 *   with an "Open setup" button; notice: an information message; mute: quiet hours hold pushes of this event type now;
 *   fetch: handed to network.request (for tests; it defaults to globalThis.fetch at send time)
 */
function createPushRuntime(deps) {
  ...
  /**
   * Push is on, the network is allowed, and at least one channel is in settings (whether its secrets are here is checked
   * when sending). Push on with the network off is "paused": events are tracked, nothing is planned.
   */
  const active = (s) => s.enabled && network.isAllowed() && s.channels.some((e) => e.enabled !== false);

  function claimDir() {
    try { return String(deps.claimDir() || ''); } catch { return ''; }
  }

  /** Quiet hours hold this event now (a failing callback mutes nothing); an urgent auto-resume counts as an error */
  function muted(ev) {
    const type = ev.type === 'autoResume' && push.isUrgent(ev) ? 'error' : ev.type;
    try { return typeof deps.mute === 'function' && deps.mute(type) === true; } catch { return false; }
  }
  ...
  /**
   * After each render.
   * @param {{ sessions?: any[], lamps?: any, quota?: any, now?: number, seq?: number, alerts?: any[],
   *   autoResume?: Map<string, any>, autoResumeEvents?: any[] }} input seq: count of real snapshots (replays not
   *   counted); a waiting needsYou check goes ahead once it grows past the count at its rescan request. alerts: threshold
   *   alerts from lib/alerts.js createThresholdTracker().update on the same data. autoResume: the auto-resume runtime's
   *   plansByKey() after its update() on the same sessions. autoResumeEvents: its outcomes not handed over yet (one
   *   handed over twice is claimed already and not sent again). Without sessions (outcomes only) the tracker, the latest
   *   snapshot and the pending waits are left as they are.
   * @returns {any[]} the tracker's events, the alerts and the auto-resume events passed in (for tests and logs)
   */
  function update(input = {}) {
    if (stopped) return [];
    const now = Number.isFinite(input.now) ? input.now : clock();
    const hasSessions = Array.isArray(input.sessions);
    if (hasSessions) latest = { sessions: input.sessions, lamps: input.lamps || null };
    const events = tracker.update({ sessions: hasSessions ? latest.sessions : undefined, lamps: latest.lamps, quota: input.quota, now });
    // The plan as it is now, when the stop is reported: a message queued or held back says what was planned then
    const plans = input.autoResume instanceof Map ? input.autoResume : null;
    if (plans) {
      for (const ev of events) {
        if (ev.type !== 'error' && ev.type !== 'limitHit') continue;
        const snap = push.autoResumeSnapshot(plans.get(ev.key));
        if (snap) ev.autoResume = snap;
      }
    }
    if (Array.isArray(input.alerts)) {
      for (const a of input.alerts) {
        if (a && push.ALERT_EVENT_TYPES.includes(a.type) && typeof a.transitionId === 'string') events.push(a);
      }
    }
    if (Array.isArray(input.autoResumeEvents)) {
      for (const o of input.autoResumeEvents) {
  ...
  async function enqueue(ev, p) {
    if (!ev || muted(ev)) return; // quiet hours: dropped, not claimed (another window outside them may send it)
  ...
  async function flush() {
    ...
    await Promise.all(batches.map(async (b) => {
      ...
      const r = await push.send(c.config, msg, { fetch: deps.fetch });
      record(c, r, b.events.length);
    }));
  }

  function record(c, r, n) {
    if (stopped || network.isOff(r)) return; // the network was turned off meanwhile: not the channel's failure
    if (r && r.ok) {
   ...
   * off. Not while the network switch is off: then it answers code 'networkOff' and nothing is sent (the setup asks
   * first). A good result also clears the channel's failure count.
   * @param {string} key
  ...
  async function test(key) {
    ...
    const r = await push.send(c.config, push.testMessage(i18n), { fetch: deps.fetch });
    if (r.ok) {
      failures.delete(key);
      warned.delete(key);
  ...
  /** Drops pending waits and queued batches (push or the network turned off, or its last channel removed) */
  function reset() {
  ...
  /** A push setting or the network switch changed */
  function onSettings() {

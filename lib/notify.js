...
const LAMP_EVENT_MAX_AGE_MS = 15 * 60e3; // createLampEventTracker: older transitions are not reported
...
/**
 * Whether the row that made the session lamp Done has a guessed done (Gemini CLI infers "done" from write timing, and the
 * guess can flip back to working when the next write arrives).
 */
function guessedDone(L, session) {
  const st = (L && L.lead && L.lead.status) || (session && session.main && session.main.status) || null;
  return !!st && st.code === S.STATUS.DONE && st.certainty === 'guess';
}

/**
 * Tracks the lamp transitions that only play a sound (lib/alerts.js): 'error' when a session's lamp turns to Error, and
 * 'done' when it goes from Working to Done that the user has not seen yet (never for a guessed done: see guessedDone).
 * - A session's first appearance (and so the first update) only seeds: nothing is reported for a state it already had.
 * - Ids derive from the data only (`error|…` / `done|…` + transitionIdOf), so every window names a transition alike and
 *   notify.claimOnce lets one window play it.
 * - A transition whose state began more than maxAgeMs ago is not reported (a laptop that slept through it).
 * @param {{ maxAgeMs?: number }} [o]
 */
function createLampEventTracker(o = {}) {
  const maxAgeMs = Number.isFinite(o && o.maxAgeMs) && o.maxAgeMs > 0 ? o.maxAgeMs : LAMP_EVENT_MAX_AGE_MS;
  const last = new Map(); // key → lamp at the last update (present keys only)

  /**
   * @param {any[]} sessions
   * @param {any} [lamps] computeLamps' result or its bySession Map (with the user's seen state)
   * @param {number} [now]
   * @returns {{ type: 'error'|'done', key: string, transitionId: string }[]}
   */
  function update(sessions, lamps, now) {
    if (!Array.isArray(sessions)) return [];
    ...
    for (const s of sessions) {
      ...
      present.add(s.key);
      const L = lampsFor(lamps, s);
      const lamp = L ? L.lamp : null;
      const was = last.get(s.key);
      last.set(s.key, lamp);
      if (was === undefined || lamp === was) continue;
      let type = null;
      if (lamp === S.LAMP.ERROR) type = 'error';
      else if (lamp === S.LAMP.DONE_UNSEEN && was === S.LAMP.WORKING && !guessedDone(L, s)) type = 'done';
      if (!type) continue;
      const since = waitSinceOf(L, s);
      if (since > 0 && t - since > maxAgeMs) continue;
      out.push({ type, key: s.key, transitionId: `${type}|${transitionIdOf(s, L)}` });
    }
    for (const k of [...last.keys()]) if (!present.has(k)) last.delete(k);
    return out;
  }
...
module.exports = {
  ...
  createNeedsYouTracker, createLampEventTracker, guessedDone, claimOnce, sendSystemNotification, plan, formatNeedsYou, itemFor, transitionIdOf,

 ...
 * 'done' when it goes from Working to Done that the user has not seen yet.
...
function createLampEventTracker(o = {}) {
  ...
  function update(sessions, lamps, now) {
    ...
    for (const s of sessions) {
      ...
      else if (lamp === S.LAMP.DONE_UNSEEN && was === S.LAMP.WORKING) type = 'done';
      if (!type) continue;
...
module.exports = {
  ...
  createNeedsYouTracker, createLampEventTracker, claimOnce, sendSystemNotification, plan, formatNeedsYou, itemFor, transitionIdOf,

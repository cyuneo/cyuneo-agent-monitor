...
/** Decimal places in an axis step (0.005 → 3, 2.5 → 1, 50 → 0), so neighbouring ticks never print the same */
function stepDecimals(step) {
  if (!(step > 0)) return 0;
  const s = String(+step.toPrecision(12));
  if (/e-/.test(s)) return Math.min(20, Number(s.split('e-')[1]) + ((s.split('e')[0].split('.')[1] || '').length));
  return (s.split('.')[1] || '').length;
}

function fmtAxisUsd(v, i18n, step) {
  try {
    const small = v < 10;
    const digits = Math.min(20, Math.max(small ? 2 : 1, stepDecimals(step)));
    return new Intl.NumberFormat(i18n.intlLocale || 'en-US', {
      style: 'currency', currency: 'USD', currencyDisplay: 'narrowSymbol',
      notation: small ? 'standard' : 'compact', maximumFractionDigits: digits, minimumFractionDigits: 0,
    }).format(v);
  } catch {
    return '$' + v;
  }
...
function buildHistoryVm(input) {
  ...
  if (hasData || r.partial) {
    ...
    const axis = (max, fmt) => {
      ...
      const step = n.ticks.length > 1 ? n.ticks[1] - n.ticks[0] : 0;
      return { top: n.top, ticks: n.ticks.map((v) => ({ value: v, text: fmt(v, step) })) };
    };
    ...
    vm.chart = {
      ...
      axis: { cost: axis(maxCost, (v, step) => fmtAxisUsd(v, i18n, step)), tokens: axis(maxTok, (v) => i18n.fmtTokens(v)) },
      bars,

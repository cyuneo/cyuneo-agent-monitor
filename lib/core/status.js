...
/** @typedef {'claude'|'codex'|'copilot'|'gemini'|'qwen'} Provider */
 ...
 *                                      // Gemini also marks running / done inferred from write timing as 'guess' (no sound / push for a guessed done)
 ...
 *  // Gemini only (Σ over every message id seen; output above already includes thoughts):
 *  input?: number,              // prompt tokens, cached part included
 *  cached?: number, thoughts?: number,
 *  tool?: number,               // tool-use prompt tokens
...
/** @typedef {{
 *  id: string,                  // Claude: 'main' or agentId; Codex: thread id; Copilot: session id / tool call id; Gemini / Qwen: session / agent id
 *  kind: 'main'|'subagent'|'workflowAgent'|'codexSubagent'|'codexReviewer'|'copilotSubagent'|'geminiSubagent'|'qwenSubagent',
 ...
 *  costEstimated?: boolean,     // Copilot / Gemini / Qwen: the cost rests on an assumed rate (e.g. Qwen's cache-hit price); Copilot: always false (costUsd null)
 ...
 *                               // running or waiting on you; Gemini: written within staleMinutes and not stopped [guess]; Qwen: runtime pid alive)
 *  liveStatus: 'busy'|'waiting'|'idle'|null,
 *  liveCertainty?: 'guess',     // Gemini only: live / liveStatus are inferred from write timing (no process registry)
 ...
 *  contextWindowSource: 'cost-state'|'model-rule'|'codex-record'|'copilot-model'|'qwen-record'|null,
 *                               // copilot-model: maxInputTokens of the chat model stored with the session; qwen-record: contextWindowSize in the record
 ...
 *                               // session file; Gemini: session-*.jsonl / .json; Qwen: <sessionId>.jsonl, sub-agents included)
 *  resume: ResumeHint[],        // Copilot / Gemini / Qwen: always []
 ...
 *  gemini: { enabled: boolean, home: string, homeSource: 'env'|'setting'|'default' },  // Gemini CLI runtime dir (~/.gemini)
 *  qwen:   { enabled: boolean, home: string, homeSource: 'env'|'setting'|'default' },  // Qwen Code home (~/.qwen)
 ...
 *  // Latest usage-limit hit seen in a session (same shape as claude; these tools keep no usage percentage on disk).
 *  // Filled by lib/monitor.js; absent in snapshots from older workers.
 *  copilot?: { lastHit: (QuotaHit & { ms: number, sessionKey: string })|null },
 *  gemini?:  { lastHit: (QuotaHit & { ms: number, sessionKey: string })|null },
 *  qwen?:    { lastHit: (QuotaHit & { ms: number, sessionKey: string })|null }
 * }} QuotaSnapshot */
...
const WINDOW_SOURCE = Object.freeze({
  ...
  QWEN_RECORD: 'qwen-record',     // contextWindowSize written in the Qwen Code record
});
...
const FAST_TOOLS = Object.freeze({
  ...
  // Gemini CLI built-in tools (same list as GEMINI_FAST_TOOLS in lib/providers/gemini.js; that module reads it from here)
  gemini: Object.freeze(['read_file', 'read_many_files', 'write_file', 'replace', 'list_directory',
    'glob', 'grep_search', 'write_todos', 'web_fetch']),
  // Qwen Code tools (same list as QWEN_FAST_TOOLS in lib/providers/qwen.js, which filters with its own copy; older Gemini CLI names included)
  qwen: Object.freeze(['read_file', 'read_many_files', 'write_file', 'edit', 'replace', 'glob', 'grep_search',

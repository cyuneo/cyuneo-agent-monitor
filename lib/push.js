...
// channels when an agent needs them, stops with an error, or hits / leaves a usage limit, and for the threshold alerts of
// lib/alerts.js (usage %, today's cost, context; worded by alerts.formatAlert with forPush). Auto-resume (DESIGN §12):
// an error / limitHit of a session with a plan says in one more line whether and when it continues (the plan as it was
// when the event was created, see autoResumeSnapshot), and each outcome (resumed / copied / failed / gaveUp) is an
// 'autoResume' event of its own (autoResumeEvent), with only the project folder name. This is the extension's only
// network access (and it goes out only through lib/network.js, which refuses every request while the user's
// agentMonitor.network.allow switch is off), so privacy comes first:
...
// No vscode dependency; fetch (passed on to network.request), clock and crypto are injectable. The extension wires it in:
...
// Threshold alerts come from lib/alerts.js (createThresholdTracker), not from the push tracker; the runtime is handed them
const ALERT_EVENT_TYPES = Object.freeze(alerts.ALERT_TYPES.slice()); // usageHigh, costDaily, contextHigh
// New types come last: this is the order of the agentMonitor.push.events properties and of the setup's event picker
const EVENT_TYPES = Object.freeze(['needsYou', 'error', 'limitHit', 'limitReset', ...ALERT_EVENT_TYPES, 'autoResume']);
// Auto-resume (lib/autoresume-runtime.js): the outcomes pushed and the plan states. A failure's error is a code
// ('cliNotFound', 'timeout' …): only a plain word is kept, and the message shows its autoresume.error.<code> text
const AUTO_RESUME_OUTCOMES = Object.freeze(['resumed', 'copied', 'failed', 'gaveUp']);
const AUTO_RESUME_STATES = Object.freeze(['scheduled', 'self', 'noReset', 'gaveUp', 'running']);
const AUTO_RESUME_ERROR_RE = /^[A-Za-z]{1,40}$/;
...
const EVENT_ORDER = Object.freeze({
  needsYou: 0, error: 1, autoResume: 2, limitHit: 3, usageHigh: 4, contextHigh: 5, costDaily: 6, limitReset: 7,
});
...
const NTFY_TAGS = Object.freeze({
  ...
  usageHigh: 'bar_chart', costDaily: 'moneybag', contextHigh: 'memo', autoResume: 'arrows_counterclockwise', test: 'bell',
});
 ...
 * code: null on success; 'networkOff' (the network switch is off: no request was made), 'config' (field / problem say
 * which), 'timeout', 'network', 'redirect', 'http' (non-2xx) or 'rejected' (2xx with an error in the body). error is
 * English and always redacted; describeError() localizes it. The request goes out through network.request() only.
 ...
 * @param {{ fetch?: Function, timeoutMs?: number, clock?: () => number, crypto?: any }} [o] fetch: handed to network.request
...
async function send(config, msg, o = {}) {
  ...
  // The switch is read again when the request is made; checked here too, so a refused send starts no timer
  if (!network.isAllowed()) return network.offResult();
  ...
  const attempt = (async () => {
    try {
      const res = await network.request(req.url, {
        method: req.method, headers: req.headers, body: req.body, redirect: 'manual', signal: ctrl ? ctrl.signal : undefined,
      }, { fetch: o && o.fetch });
      if (network.isOff(res)) return res; // turned off in between: nothing went out
...
function describeError(result, t, o = {}) {
  ...
  switch (r.code) {
    ...
    case 'networkOff': return tr('push.err.networkOff');
...
// Providers whose QuotaSnapshot entry is { lastHit } (the latest usage-limit error seen in a session)
const LAST_HIT_PROVIDERS = Object.freeze(['claude', 'copilot']);

/**
 * Current usage-limit hits from a QuotaSnapshot (lib/core/quota.js, filled by monitor.js): the lastHit of Claude and Copilot,
 * and Codex when rate_limit_reached_type is set or a window is at 100 %. The id derives from the data
 * only (reset time, else the hit time), so every window names the same hit alike.
 */
function limitHitsOf(quota) {
  const out = [];
  for (const provider of LAST_HIT_PROVIDERS) {
    const hit = quota && quota[provider] && quota[provider].lastHit;
    if (!hit || !Number.isFinite(hit.ms) || !(hit.ms > 0)) continue;
    const resetAt = Number.isFinite(hit.resetsAtMs) && hit.resetsAtMs > 0 ? Math.round(hit.resetsAtMs) : null;
    out.push({ provider, resetAt, at: Math.round(hit.ms), sessionKey: typeof hit.sessionKey === 'string' ? hit.sessionKey : null });
  }
...
/** A count (attempt, max) as a whole number, or null */
function countOf(v) {
  return Number.isFinite(v) && v >= 0 ? Math.round(v) : null;
}

/**
 * What an error / limitHit message may say about its session's auto-resume plan (DESIGN §12.5 Plan): the runtime copies
 * it onto the event when the event is created (ev.autoResume), so a message sent later still tells the plan as it was
 * at the stop. Only the state, the time, the attempt and the limit, and the enabled folder's name; never its path, the
 * session id or the stop id. null when it is not a plan.
 * @param {any} plan
 * @returns {{ state: string, atMs: number|null, attempt: number|null, max: number|null, projectName: string|null }|null}
 */
function autoResumeSnapshot(plan) {
  if (!plan || typeof plan !== 'object' || !AUTO_RESUME_STATES.includes(plan.state)) return null;
  return {
    state: plan.state,
    atMs: Number.isFinite(plan.atMs) && plan.atMs > 0 ? Math.round(plan.atMs) : null,
    attempt: countOf(plan.attempt),
    max: countOf(plan.max),
    projectName: typeof plan.projectName === 'string' && plan.projectName ? plan.projectName : null,
  };
}

/**
 * Push event for an auto-resume outcome (DESIGN §12.5 Outcome, from the auto-resume runtime's onOutcome); null when it
 * is not one. The message shows the project folder name, the attempt, the limit and (failed) the reason; the event keeps
 * nothing else of the outcome (no folder path, session or copy id). transitionId, the claim id:
 * autoResume|<sessionId>|<attempt>|<outcome>, and for resumed / copied / failed |<atMs> too: only the window that ran the
 * resume reports those, and the attempt number starts again after a chat finishes a turn, so the same attempt of the same
 * session can come back within the claim markers' 24 hours. gaveUp leaves the time out, so windows that each report the
 * same one claim the same id.
 * @param {any} o
 */
function autoResumeEvent(o) {
  if (!o || typeof o !== 'object' || !AUTO_RESUME_OUTCOMES.includes(o.outcome)) return null;
  const sessionId = typeof o.sessionId === 'string' ? o.sessionId : '';
  if (!sessionId) return null;
  const attempt = countOf(o.attempt);
  const at = Number.isFinite(o.atMs) && o.atMs > 0 ? Math.round(o.atMs) : null;
  const tail = o.outcome !== 'gaveUp' && at != null ? `|${at}` : '';
  const name = typeof o.projectName === 'string' && o.projectName ? o.projectName : projectOf({ cwd: o.project });
  return {
    type: 'autoResume',
    key: typeof o.key === 'string' && o.key ? o.key : `claude:${sessionId}`,
    transitionId: `autoResume|${sessionId}|${attempt == null ? '' : attempt}|${o.outcome}${tail}`,
    outcome: o.outcome,
    project: name || null,
    attempt,
    max: countOf(o.max),
    error: typeof o.error === 'string' && AUTO_RESUME_ERROR_RE.test(o.error) ? o.error : null,
    title: '',
    titleSource: null,
    agentName: null,
    provider: 'claude',
...
/**
 * Worth waking the phone for (high priority): a chat that needs you or stopped with an error, an auto-resume that failed
 * or gave up. The runtime also lets such an auto-resume through quiet hours' allowErrors, like an error.
 * @param {any} e
 */
function isUrgent(e) {
  if (!e) return false;
  if (e.type === 'needsYou' || e.type === 'error') return true;
  return e.type === 'autoResume' && (e.outcome === 'failed' || e.outcome === 'gaveUp');
}

/** A known event type (an auto-resume event only with a known outcome) */
function isEvent(e) {
  return !!e && EVENT_TYPES.includes(e.type) && (e.type !== 'autoResume' || AUTO_RESUME_OUTCOMES.includes(e.outcome));
}
...
/**
 * Push settings with defaults: off; every event on (the threshold alerts fire only when their threshold is set, see
 * lib/alerts.js); needsYou waits 30 s; chat titles left out.
 * @param {any} raw { enabled, events: { needsYou, error, limitHit, limitReset, usageHigh, costDaily, contextHigh,
 *   autoResume }, delaySeconds, includeTitle }
 */
 ...
 * - 'send': go now (errors, limits, threshold alerts and auto-resume outcomes at once); claimId is for notify.claimOnce
 *   so only one window sends it.
...
function plan(o = {}) {
  ...
  if (!isEvent(event) || typeof event.transitionId !== 'string') {
...
/**
 * The line an error / limitHit message adds for a session with an auto-resume plan (its snapshot, see
 * autoResumeSnapshot); '' when there is none or nothing true to say: 'running' is a resume of an earlier stop under way
 * (this stop has no plan yet), and a time that cannot be formatted is not guessed.
 */
function autoResumeLine(plan, tr, clock) {
  if (!plan || typeof plan !== 'object') return '';
  const time = Number.isFinite(plan.atMs) && clock ? clock(plan.atMs) : '';
  if (plan.state === 'scheduled') return time ? tr('autoresume.push.line.scheduled', { time, n: plan.attempt, max: plan.max }) : '';
  if (plan.state === 'self') return time ? tr('autoresume.push.line.self', { time }) : '';
  if (plan.state === 'noReset') return tr('autoresume.push.line.noReset');
  if (plan.state === 'gaveUp') return tr('autoresume.push.line.gaveUp', { max: plan.max });
  return '';
}

/** Title and body for one event; note: the auto-resume line of an error / limitHit ('' for none) */
function describeEvent(e, tr, o) {
  if (ALERT_EVENT_TYPES.includes(e.type)) {
    // Threshold alerts: alerts.formatAlert with the push rules (a real chat title only with includeTitle, no cost amounts)
    const m = alerts.formatAlert(e, o.i18n || tr, {
      forPush: true, includeTitle: !!o.includeTitle, fmtClock: o.clock || undefined, now: o.now,
    });
    return { title: m.title, body: m.body, chat: '', note: '' };
  }
  const project = cleanText(e.project, NAME_MAX);
  if (e.type === 'autoResume') {
    // Only the project folder name, the attempt and (failed) the reason: never the prompt, a path or a session id
    const key = typeof e.error === 'string' && AUTO_RESUME_ERROR_RE.test(e.error) ? `autoresume.error.${e.error}` : '';
    const text = key ? tr(key) : '';
    const reason = text && text !== key ? text : ''; // a code with no text of its own is left out
    const vars = { project, n: e.attempt, max: e.max, reason };
    return { title: tr(`autoresume.push.title.${e.outcome}`, vars), body: tr(`autoresume.push.body.${e.outcome}`, vars), chat: '', note: '' };
  }
  ...
  const note = e.type === 'error' || e.type === 'limitHit' ? autoResumeLine(e.autoResume, tr, o.clock) : '';
  if (e.type === 'needsYou' || e.type === 'error') {
    ...
    return { title, body, chat, note };
  }
  const provider = providerName(tr, e.provider);
  if (e.type === 'limitHit') {
    const reset = Number.isFinite(e.resetAt) && o.clock ? o.clock(e.resetAt) : '';
    // A limit is not tied to a project, but its auto-resume line speaks of "this project": then the title names it
    const where = note ? project || cleanText(e.autoResume.projectName, NAME_MAX) : '';
    return {
      title: where ? tr('push.msg.limitHit.titleProject', { provider, project: where }) : tr('push.msg.limitHit.title', { provider }),
      body: reset ? tr('push.msg.limitHit.body', { reset }) : tr('push.msg.limitHit.bodyUnknown'),
      chat: '',
      note,
    };
  }
  return { title: tr('push.msg.limitReset.title', { provider }), body: tr('push.msg.limitReset.body'), chat: '', note: '' };
}

/**
 * Push message for one or more events (a coalesced batch).
 * Default content: the project folder name and the state (for limits: the provider and the reset time; for threshold
 * alerts: the percentage, and for today's cost only that the budget was passed; for auto-resume outcomes: the attempt
 * and, when it failed, the reason). An error / limitHit with an auto-resume plan adds one line: when the chat continues
 * (or why it does not); in a batch that goes on the event's own line, as does an auto-resume event's body (its title
 * names no project). Chat titles and agent names only with
 * includeTitle. Never prompts, code, paths, session ids, token counts or cost.
 * @param {any[]} events
...
function formatPush(events, t, o = {}) {
  ...
  const list = (Array.isArray(events) ? events : [events])
    .filter((e) => isEvent(e) && !seen.has(e.transitionId) && seen.add(e.transitionId))
    .sort((a, b) => EVENT_ORDER[a.type] - EVENT_ORDER[b.type]);
  if (!list.length) return { title: APP_NAME, body: '', priority: 'normal', event: '' };
  const opts = { includeTitle: !!o.includeTitle, clock, now, i18n: t && typeof t === 'object' ? t : null };
  const priority = list.some(isUrgent) ? 'high' : 'normal';
  if (list.length === 1) {
    const d = describeEvent(list[0], tr, opts);
    return { title: d.title, body: d.note ? `${d.body}\n${d.note}` : d.body, priority, event: list[0].type };
  }
  const lines = list.slice(0, BATCH_LINES_MAX).map((e) => {
    const d = describeEvent(e, tr, opts);
    const line = d.chat ? tr('push.msg.batch.lineTitle', { line: d.title, title: d.chat }) : d.title;
    // An auto-resume title names no project: its body (project, attempt or reason) goes on its line
    const note = e.type === 'autoResume' ? d.body : d.note;
    return note ? tr('push.msg.batch.lineNote', { line, note }) : line;
  });
...
module.exports = {
  APP_NAME, EVENT_TYPES, ALERT_EVENT_TYPES, AUTO_RESUME_OUTCOMES, CLAIM_PREFIX, DEFAULT_TIMEOUT_MS, DEFAULT_DELAY_SECONDS,
  // channels
  ...
  autoResumeSnapshot, autoResumeEvent, isUrgent,
  // text

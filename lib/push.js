...
// lib/alerts.js (usage %, today's cost, context; worded by alerts.formatAlert with forPush). This is the extension's only
...
const EVENT_TYPES = Object.freeze(['needsYou', 'error', 'limitHit', 'limitReset', ...ALERT_EVENT_TYPES]);
...
const EVENT_ORDER = Object.freeze({
  needsYou: 0, error: 1, limitHit: 2, usageHigh: 3, contextHigh: 4, costDaily: 5, limitReset: 6,
});
...
const NTFY_TAGS = Object.freeze({
  ...
  usageHigh: 'bar_chart', costDaily: 'moneybag', contextHigh: 'memo', test: 'bell',
});
 ...
 * - 'send': go now (errors, limits and threshold alerts at once); claimId is for notify.claimOnce so only one window
 *   sends it.
...
function describeEvent(e, tr, o) {
  if (ALERT_EVENT_TYPES.includes(e.type)) {
    ...
    return { title: m.title, body: m.body, chat: '' };
  }
 ...
 * alerts: the percentage, and for today's cost only that the budget was passed). Chat titles and agent names only with
 * includeTitle. Never prompts, code, paths, token counts or cost.
 * @param {any[]} events
...
module.exports = {
  APP_NAME, EVENT_TYPES, ALERT_EVENT_TYPES, CLAIM_PREFIX, DEFAULT_TIMEOUT_MS, DEFAULT_DELAY_SECONDS,
  // channels

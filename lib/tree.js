...
const { contextZone, sessionRowVm, ZONE_MARK, autoResumeInfo } = require('./agents-view');
...
class AgentTreeProvider {
  /**
   * @param {{ i18n: any, now?: () => number, hideCompleted?: boolean, showCost?: boolean, autoResume?: any }} o
   *   autoResume: the auto-resume provider (same object as for AgentsViewProvider; lib/agents-view.js autoResumeInfo):
   *   session contextValue gets bgResumable / autoResumePending, the session tooltip the plan line; absent → neither
   */
  constructor(o) {
    this.i18n = o.i18n;
    ...
    this.autoResume = o.autoResume && typeof o.autoResume === 'object' ? o.autoResume : null;
  ...
  rebuild() {
    ...
    const sessNode = (s) => {
      ...
      return sync(s.key, 'session', null, s, (node) => {
        ...
        return sessionVm(s, L, node.open, i18n, now, this.hints, autoResumeInfo(this.autoResume, s));
      }, (node) => {
  ...
  tooltip(node, now = this.now()) {
    ...
    if (node.kind === 'session') {
      return sessionTooltip(node.data, node.extra && node.extra.lamps, i18n, {
        now, showCost: this.showCost, hints: this.hints, autoResumeInfo: autoResumeInfo(this.autoResume, node.data),
      });
    }
...
// and bgResumable / autoResumePending from the session's AutoResumeInfo (ar)
function sessionVm(s, L, open, i18n, now, hints, ar) {
  const vm = sessionRowVm(s, L, i18n, now, hints, ar);
 ...
 * o.autoResumeInfo: the session's AutoResumeInfo; adds the auto-resume plan line.
 * @param {any} s Session
 * @param {any} [L] result of sessionLamps
 */
function sessionTooltip(s, L, i18n, o = {}) {
  const lamps = L || sessionLamps(s);
  const rows = fmt.formatSessionTooltip(s, i18n, { lamps, now: o.now, showCost: o.showCost, autoResumeInfo: o.autoResumeInfo });

...
// Windows: replacing a file that another process has open (an editor, antivirus, the search indexer) fails briefly with
// EPERM / EBUSY / EACCES. Retry a few times with short pauses (about 0.2 s in total); any other error is thrown at once.
const RENAME_RETRY_CODES = new Set(['EPERM', 'EBUSY', 'EACCES']);
const RENAME_RETRY_MS = [10, 30, 60, 100];

function sleepSync(ms) {
  try { Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, ms); } catch { /* no Atomics.wait: retry at once */ }
}

/** fs.renameSync with a short retry on transient sharing errors; the last error is thrown if every attempt fails */
function renameWithRetry(from, to) {
  for (let i = 0; ; i++) {
    try {
      ...
      if (i >= RENAME_RETRY_MS.length || !err || !RENAME_RETRY_CODES.has(err.code)) throw err;
...
function activateAutoCompact(context, deps) {
  ...
  async function run(arg) {
    ...
    if (it.action.type === 'guide') {
      try { await vscode.env.openExternal(vscode.Uri.parse(P.guideUrl(i18n && i18n.locale))); } catch (err) { log(`openExternal: ${err && err.message}`); }
      return;
    }

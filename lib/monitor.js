...
// Copilot Chat), merges them into Snapshot v2, applies the activity-window filter, attaches details
...
class Monitor {
   ...
   *   other provider modules by name (modules.copilot), the storage module (null = unavailable), the
  ...
  // Copilot: the main session file, plus sub-agent files other than it (Copilot keeps sub-agents in the same file, so
  // usually none); no file backups
  ...
  snapshot(now = Date.now()) {
    ...
    // Codex, Copilot: each in isolation
   ...
   * @param {string} key 'claude:<id>' / 'codex:<id>' / 'copilot:<id>'
   */
  detail(key) {

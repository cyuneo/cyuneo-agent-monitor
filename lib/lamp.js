...
// - Every provider feeds the same lamps. Guessed statuses (certainty 'guess': Codex / Claude / Qwen "may be waiting for
//   approval", and Gemini CLI's running / done inferred from write timing) get exactly the lamp of the same certain status,
//   never a stronger one: a guessed done is DoneUnseen like any done. The UI marks guesses in the text (format.js), not here.
// - Only Claude has a process registry. Copilot / Qwen / Gemini also fill liveStatus, but derive it from the same status,
 ...
 * provider tagged certainty 'guess' (Gemini CLI infers running / done from write timing).

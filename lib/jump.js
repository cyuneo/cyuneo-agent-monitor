...
//   claudeCli, codexCli, geminiCli, qwenCli
//                  CLI sessions: find the agent's process (Claude: registry pid; Qwen: runtime.json pid; Codex: the process
//                  that holds the rollout file open, else like Gemini; Gemini: search processes by command name and working
//                  directory), walk the process tree up and match an ancestor against the shell pids of the integrated
//                  terminals of this or another window → terminal.show() there. Last resort, macOS only (not in a remote
//                  window): a process in Terminal.app or iTerm2 → select its tab by tty with AppleScript, after a one-time
//                  confirmation (macOS asks for Automation permission). A registry pid (Claude, Qwen) whose process started
//                  after the session registered it belongs to another program now (pid reuse): not running
...
// call on Windows, each with a timeout (plus, for Codex / Gemini, one `lsof` on macOS or /proc on Linux for the working
// directories or the holder of the rollout file). Session content is never read here; only the session fields the providers
// already expose, the Claude live registry and Qwen's runtime.json (both of which the providers read anyway).
//
...
//
// API:
//   createJumper({ vscode, platform?, claudeHome?, claudeLive?, qwenPid?, localInfo?, windows?, requestJump?, cancelJump?,
...
/**
 * Working directories of a few processes (the Codex / Gemini candidates): macOS one lsof call, Linux /proc/<pid>/cwd.
...
/**
 * Whether a pid taken from a registry (Claude's live registry, Qwen's runtime.json) now belongs to a process that started
...
// ---------------------------------------------------------------------------
// Finding the agent process of a CLI without a registry (Codex, Gemini)
// ---------------------------------------------------------------------------
...
const AGENT_PROCESS = Object.freeze({
  ...
  gemini: Object.freeze({
    name: /^gemini(\.(exe|cmd|js|mjs|cjs))?$/,
    pkg: /[\\/]@google[\\/]gemini-cli[\\/]|[\\/]gemini-cli[\\/](dist|bundle)[\\/]/i,
    skip: Object.freeze([]),
  }),
});
...
/**
 * The tool's CLI processes. A launcher and the process it starts (the npm wrapper and the native codex binary, Gemini's
 * relaunched child) count once: only the outermost one is kept (both sit under the same terminal).
 */
...
async function qwenPid(s, env) {
  const v = env.qwenPid(s);
  const pid = v && typeof v === 'object' ? v.pid : v;
  if (!Number.isInteger(pid) || pid <= 0) return { reason: REASON.NOT_RUNNING };
  return { pid, startedMs: v && typeof v === 'object' ? v.startedMs : null };
}
...
const RESOLVERS = Object.freeze([
  ...
  { id: 'geminiCli', test: (s) => s.provider === 'gemini', live: 'unknown', resolve: (s, env) => resolveTerminal(s, env, searchPid('gemini')) },
  { id: 'qwenCli', test: (s) => s.provider === 'qwen', live: 'certain', resolve: (s, env) => resolveTerminal(s, env, qwenPid) },
...
// ---------------------------------------------------------------------------
// Qwen: pid from <sessionId>.runtime.json next to the transcript
// ---------------------------------------------------------------------------

/** → { pid, startedMs } from runtime.json while that pid is alive, else null */
function qwenPidOf(s, o = {}) {
  const t = s && typeof s.transcript === 'string' ? s.transcript : '';
  if (!t || !nodePath.isAbsolute(t) || !s.id || /[\\/]/.test(String(s.id))) return null;
  const qwen = require('./providers/qwen');
  const rt = qwen.readRuntime(nodePath.join(nodePath.dirname(t), `${s.id}.runtime.json`));
  if (qwen.aliveOf(rt, s.id, o.hostname || nodeOs.hostname(), o.isAlive) !== true) return null;
  const st = rt.started_at;
  const startedMs = typeof st === 'number' && Number.isFinite(st) ? (st > 1e12 ? st : st * 1000) : typeof st === 'string' ? Date.parse(st) : NaN;
  return { pid: Number(rt.pid), startedMs: Number.isFinite(startedMs) ? startedMs : null };
}
 ...
 *   qwenPid?: (session: any) => number|null,        // replaces the runtime.json read (tests)
...
function createJumper(deps = {}) {
  ...
  async function makeEnv() {
    ...
    return {
      ...
      claudeLive,
      qwenPid: deps.qwenPid || ((s) => qwenPidOf(s)),
 ...
 *   claudeLive: (sessionId: string) => any, qwenPid: (session: any) => number|null,
...
module.exports = {
  ...
  listProcesses, processCwds, fileHolders, qwenPidOf,
  _internal: {

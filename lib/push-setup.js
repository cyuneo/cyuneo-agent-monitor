...
// One QuickPick, titled with the current state (on / paused / off, channels in use, network access), that comes back
// after each quick action:
...
// - Network access (agentMonitor.network.allow, lib/network.js): the title shows it, and one item allows or blocks it.
//   While it is off, adding a channel, turning push on or sending a test first shows a modal that says what would be sent
//   and where; only its "Allow network access" button turns the switch on (in user settings). Push that is on while the
//   network is off shows as paused; push.enabled is left as it is.
...
async function runPushSetup(deps) {
  ...
  for (;;) {
    ...
    items.push(netOn
      ? { label: icon('circle-slash', t('push.net.block')), description: t('push.net.blockHint'), action: 'netBlock' }
      : { label: icon('globe', t('push.net.allow')), description: t(s.enabled ? 'push.net.pausedHint' : 'push.net.offHint'), action: 'netAllow' });
    ...
    const pushState = !s.enabled ? 'push.ui.stateOff' : netOn ? 'push.ui.stateOn' : 'push.net.statePaused';
    const netState = netOn ? 'push.net.stateAllowed' : 'push.net.stateOff';
    const title = t('push.ui.menuTitle', { state: t(pushState), n: inUse.length, network: t(netState) });
    ...
    switch (pick.action) {
      ...
      case 'on':
        ...
        if (!(await allowNetwork(t('push.net.sends', { channel: listOf((inUse.length ? inUse : list).map((c) => c.name)) })))) break;
      ...
      case 'netAllow': await deps.write('network.allow', true); break;
      case 'netBlock': await deps.write('network.allow', false); break; // push.enabled stays as it is: push is paused
  ...
  async function addChannel(s, turnOn = false) {
    ...
    if (!network.isAllowed()) {
      // One modal instead of the privacy notice: what is sent and where, and that the network is off until allowed
      if (!(await allowNetwork(t('push.net.sends', { channel: label })))) return false;
    } else if (!acked.includes(ch.id)) {
  ...
  /**
   * The network switch is off: a modal says what would be sent and where (what), with "Allow network access". Only that
   * button turns the switch on, in user settings; anything else leaves it off.
   * @returns {Promise<boolean>} true when the network is allowed (already, or now)
   */
  async function allowNetwork(what) {
    if (network.isAllowed()) return true;
    const go = t('push.net.allow');
    const b = await win.showInformationMessage(t('push.net.askTitle'), { modal: true, detail: `${what}\n\n${t('push.net.askDetail')}` }, go);
    if (b !== go) return false;
    await deps.write('network.allow', true);
    return true;
  }

  async function testOne(c) {
    if (!(await allowNetwork(t('push.net.test', { channel: c.name })))) return;
  ...
  async function testAll(list) {
    ...
    if (!(await allowNetwork(t('push.net.test', { channel: listOf(list.map((c) => c.name)) })))) return;

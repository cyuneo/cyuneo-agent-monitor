...
const GOTO_COMMAND = 'agentMonitor.goToChat';
const SEP = F.SEP;
...
// when: the item appears only if the session carries this flag (compactable / handoff / autoCompact / resumable /
// bgResumable / autoResumePending in data-vscode-context).
const SESSION_MENU = Object.freeze([
  { command: 'agentMonitor.goToChat', group: '0_goto', when: null },
  { command: 'agentMonitor.compact', group: '1_session', when: 'compactable' },
  { command: 'agentMonitor.handoff', group: '1_session', when: 'handoff' },
  { command: 'agentMonitor.setAutoCompact', group: '1_session', when: 'autoCompact' },
  { command: 'agentMonitor.copyResume', group: '1_session', when: 'resumable' },
  { command: 'agentMonitor.autoResume.now', group: '1_session', when: 'bgResumable' },
  { command: 'agentMonitor.autoResume.cancel', group: '1_session', when: 'autoResumePending' },
 ...
 * @param {{ compactable?: boolean, resumable?: boolean, handoff?: boolean, autoCompact?: boolean,
 *   bgResumable?: boolean, autoResumePending?: boolean }} flags
 */
...
function agentTip(agent, status, lamp, i18n, o) {
  ...
  const ctx = F.formatContext(F.agentContextTokens(agent, o.provider), i18n);
  // Token counts never recorded (Copilot sub-agents, Copilot chats without usage): one note instead of three "0 tokens" lines
  const unknown = ctx.unknown;
  const counts = [
    i18n.t('count.apiCalls', { n: unknown && !(tk.apiCalls > 0) ? i18n.fmtNum(null) : tk.apiCalls || 0 }),
  ...
  const c = o.showCost ? F.formatAgentCost(agent, i18n, { provider: o.provider }) : null;
  const cost = c ? c.label + ': ' + c.full : '';
  return lines(
    // The main agent's kind would just repeat its name ("Main agent · Main agent")
    agent.kind === 'main' ? F.formatAgentName(agent, i18n) : F.formatAgentName(agent, i18n) + SEP + F.formatAgentKind(agent, i18n),
    ...
    unknown ? F.formatTokensUnknownNote(agent, o.provider, i18n) : '',
    unknown ? '' : i18n.t('webview.tip.latestCall', { n: i18n.fmtNum(tk.display || 0) }),
    unknown ? '' : i18n.t('webview.tip.output', { n: i18n.fmtNum(tk.output || 0) }),
    unknown ? '' : i18n.t('webview.tip.processed', { n: i18n.fmtNum(tk.processed || 0) }),
    counts,
    cost,
...
function agentRowVm(r, L, i18n, o) {
  ...
  const tk = a.tokens || {};
  const unknown = F.tokensUnknown(a, o.provider);
  const cost = o.showCost ? F.formatAgentCost(a, i18n, { provider: o.provider }) : null;
  return {
    id: r.id,
    kind: r.kind,
    depth: r.depth,
    ...
    guess: lampLib.isGuessStatus(status),
    stepText: F.formatStep(a.step, status, i18n, o.now, { withDur: true }),
    // Counts never recorded read "—", not "0"; the reason is on hover
    tokensText: unknown ? i18n.fmtTokens(null) : i18n.fmtTokens(tk.display || 0),
    tokensTip: unknown ? F.formatTokensUnknownNote(a, o.provider, i18n) : '',
    // Without a price the cell shows only "—"; the full explanation (no public pricing / estimate / Copilot credits) goes in the hover tooltip
    costText: cost ? cost.cell : '',
    costTip: cost ? cost.full : '',
...
function detailVm(agent, d, i18n, o) {
  ...
  const base = { loaded: !!d, countsText: counts, canOpen: isTranscriptFile(agent.file) };
...
function contextVm(s, i18n, o) {
  ...
  // Not recorded: "— / 128K window" with no bar or percentage (ctx.ratio / pctText are empty)
  const usedText = ctx.unknown ? i18n.fmtTokens(null) : i18n.fmtTokens(ctx.used);
  const text = ctx.limit
    ? i18n.t(ofCompact ? 'webview.ctx.ofCompact' : 'webview.ctx.ofWindow', { used: usedText, limit: i18n.fmtTokens(ctx.limit) })
    : ctx.usageText;
  ...
  const tipParts = [text, F.formatTokensUnknownNote(s.main, s.provider, i18n), ctx.pctOfWindowText, ctx.remainText, ...ctx.notes, ...src.lines];
...
function bannersVm(s, L, i18n, o) {
  ...
  // The account's latest limit hit (Claude / Copilot), while it still matters: Claude until its reset
  // time, Copilot (no reset time recorded) for an hour; not when this session's own status already says it
  const last = F.LAST_HIT_PROVIDERS.includes(s.provider) && !quotaSt && q[s.provider] ? q[s.provider].lastHit : null;
  const recent = !!last && (s.provider === 'claude' ? fin(last.resetsAtMs) && last.resetsAtMs > o.now : F.isRecentHit(last, o.now));
  if (recent) {
    out.push({
      tone: 'info', icon: 'info',
      text: F.formatLastHit(s.provider, last, i18n, o.now),
      detail: '', tip: s.provider === 'claude' ? i18n.t('quota.claude.noPercent') : i18n.t('quota.lastHit.tip'),
    });
  }
  if (o.ctx && o.ctx.many) {
...
/**
 * A session's AutoResumeInfo from the extension's auto-resume provider
 * ({ infoFor(session), now(sessionKey), cancel(sessionKey), setProjectFor(session, on) }, docs/DESIGN.md §12).
 * null when there is no provider, the session is not a Claude Code chat, or the provider fails: no auto-resume UI at all.
 * @param {any} provider
 * @param {any} s Session
 */
function autoResumeInfo(provider, s) {
  if (!provider || typeof provider.infoFor !== 'function' || !s) return null;
  let info = null;
  try { info = provider.infoFor(s); } catch { return null; }
  return F.autoResumeApplies(s, info) ? info : null;
}

// Plan line icon by Plan.state (a plan that can't run right now, Plan.unavailable, gets the warning icon)
const AUTORESUME_ICON = { scheduled: 'clock', self: 'clock', running: 'sync', noReset: 'warning', gaveUp: 'warning' };

/**
 * Auto-resume block in the resume area: project switch, plan line, [Continue in background] [Cancel] and the note that
 * a chat still open is continued as a copy. Shown for every Claude Code chat, even with nothing planned (so the project
 * switch can be reached); null for other tools or without a provider. heading is the block's accessible name (the
 * project line already starts with "Auto-resume is on / off").
 * o.autoResumeBusy: sessions whose "Continue in background" is still starting (the button is hidden meanwhile).
 */
function autoResumeVm(s, i18n, o) {
  const info = autoResumeInfo(o.autoResume, s);
  if (!info) return null;
  const f = F.formatAutoResume(info, i18n, o.now);
  const flags = F.autoResumeFlags(info);
  const busy = !!(o.autoResumeBusy && o.autoResumeBusy.has(s.key));
  const canNow = flags.bgResumable && !busy;
  const warn = !!f.unavailableText || f.state === 'noReset' || f.state === 'gaveUp';
  // The copy note only matters while the chat is open (only then does Claude Code start a copy) and something is about
  // to run in the background; it is also in the tooltips (a narrow panel hides the note line)
  const noteText = s.live && (canNow || f.state === 'scheduled' || f.state === 'running') ? f.copyNote : '';
  return {
    heading: i18n.t('autoresume.section'),
    projectOn: f.projectOn,
    projectText: f.projectText,
    projectTip: lines(f.projectText, f.howNote),
    toggleText: i18n.t(f.projectOn ? 'autoresume.project.turnOff' : 'autoresume.project.turnOn'),
    toggleTip: lines(f.howNote, f.claudeOnlyNote),
    state: f.state,
    planText: f.planText,
    planTip: lines(f.planText, noteText, f.howNote),
    icon: f.unavailableText ? 'warning' : (AUTORESUME_ICON[f.state] || 'clock'),
    tone: warn ? 'warning' : '',
    canNow,
    nowText: canNow ? i18n.t('autoresume.btn.now') : '',
    nowTip: canNow ? f.copyNote : '',
    canCancel: flags.autoResumePending,
    cancelText: flags.autoResumePending ? i18n.t('autoresume.btn.cancel') : '',
    noteText,
  };
}
...
function autoCompactVm(s, i18n, o) {
  ...
  // Only Claude Code and Codex have an auto-compact setting this extension can read and change
  if (!F.hasCompactPoint(s.provider)) return null;
...
function sessionVm(s, L, i18n, o) {
  ...
  return {
    ...
    metaTip: lines(
      ...
      i18n.t('tip.entry') + ': ' + F.entryLabel(s.entry, i18n) + SEP + F.formatLive(s, i18n),
      fin(s.startedMs) ? i18n.t('tip.started') + ': ' + i18n.fmtDateTime(s.startedMs) : '',
    ),
    statusText,
    statusTip: lines(F.lampLabel(lamp, i18n), F.formatStatusNote(leadStatus, i18n), errMsg, F.formatProviderNote(s.provider, i18n)),
    guess: lampLib.isGuessStatus(leadStatus),
    context: ctx,
    compactable,
    goTo: jumpLib.worthTrying(s, o.now), // the Go to button; the context menu and double-click always try (and say why not)
    ...
    costTip: o.showCost
      ? (cost.fromClaudeCode
        ...
        : lines(cost.label, F.formatCostNote(i18n, s.provider)))
      : '',
    // Today's total is Claude Code + Codex only (lib/core/daily.js); say so, since it also shows on other tools' sessions
    todayTip: o.showCost ? lines(i18n.t('cost.label') + SEP + i18n.t('cost.today'), ...today.lines, i18n.t('cost.today.scope'), F.formatCostNote(i18n)) : '',
    banners: bannersVm(s, L, i18n, { ...o, ctx }),
    resume: resumeVm(s, i18n, o),
    autoResume: autoResumeVm(s, i18n, o),
  };
}
 ...
 *   autoResume?: any,                  // auto-resume provider (see autoResumeInfo); absent → no auto-resume block
 *   autoResumeBusy?: Set<string>,      // sessions whose "Continue in background" is still starting
 * }} input
...
function buildView(input) {
  ...
  const base = { type: 'render', v: 1, now, showCost, sessionKey: null, emptyText: '', emptyAction: null, session: null, rows: [], detail: {}, note: '', costHead: null };
  ...
  const o = {
    ...
    autoResumeBusy: input.autoResumeBusy instanceof Set ? input.autoResumeBusy : null,
  };
  ...
  const vm = {
    ...
    // Copilot bills in credits: the cost column is headed "Credits" and holds plain numbers (null → the default "Cost" header)
    costHead: s.provider === 'copilot' ? { text: i18n.t('webview.col.credits'), tip: F.formatCostNote(i18n, s.provider) } : null,
  };
  return { vm, rowAgents };
}
 ...
 * description = {provider} · one-line status · context percent [zone marker] (lib/format.js formatSessionRow).
 * @param {any} s Session
 * @param {any} L result of sessionLamps
 * @param {any} i18n
 * @param {number} now
 * @param {{ start?: number, act?: number }} [hints] contextHintStart / contextHintAct
 * @param {any} [autoResume] the session's AutoResumeInfo (autoResumeInfo()); adds the bgResumable / autoResumePending flags
 * @returns {{ label: string, description: string, icon: string, color: string, contextValue: string, a11y: string, mark: string }}
 */
function sessionRowVm(s, L, i18n, now, hints, autoResume) {
  ...
  return {
    ...
    icon: look.shape,
    color: look.colorId,
    contextValue: F.sessionContextValue(s, L.lamp, autoResume),
    a11y,
...
/**
 * contextValue (space-separated flags) → the flags the menu needs. handoff / autoCompact: only Claude Code and Codex
 * sessions have a handoff note (lib/compact.js) and an auto-compact setting (lib/autocompact.js).
 * bgResumable / autoResumePending come from the auto-resume provider (format.js sessionContextValue).
 */
function flagsOf(contextValue) {
  const set = new Set(String(contextValue || '').split(/\s+/));
  const cc = set.has('provider-claude') || set.has('provider-codex');
  return {
    compactable: set.has('compactable'), resumable: set.has('resumable'), handoff: cc, autoCompact: cc,
    bgResumable: set.has('bgResumable'), autoResumePending: set.has('autoResumePending'),
  };
}
 ...
 * @param {any} i18n
 * @param {{ now?: number, showCost?: boolean, hints?: { start?: number, act?: number }, autoResumeInfo?: any }} [o]
 *   autoResumeInfo: the session's AutoResumeInfo; adds the auto-resume plan line
 */
function sessionTipText(s, L, i18n, o = {}) {
  const lamps = L || lampLib.sessionLamps(s);
  const rows = F.formatSessionTooltip(s, i18n, { lamps, now: o.now, showCost: o.showCost, autoResumeInfo: o.autoResumeInfo });
  ...
  if (o.showCost !== false && rows.some(([k]) => k === i18n.t('cost.label') || k === i18n.t('cost.credits.label'))) {
    out.push('', F.formatCostNote(i18n, s.provider));
  }
  return out.join('\n');
}
...
function sessionListRow(s, L, group, i18n, o) {
  const ar = autoResumeInfo(o.autoResume, s);
  const vm = sessionRowVm(s, L, i18n, o.now, o.hints, ar);
  const flags = flagsOf(vm.contextValue);
  const look = lampBits(L.lamp);
  // The VS Code context menu reads data-vscode-context: webviewSection / sessionKey / the menu flags; suppress the default Cut / Copy / Paste items
  const context = {
    webviewSection: 'session', sessionKey: s.key,
    compactable: flags.compactable, resumable: flags.resumable, handoff: flags.handoff, autoCompact: flags.autoCompact,
    bgResumable: flags.bgResumable, autoResumePending: flags.autoResumePending,
  ...
  return {
    ...
    shape: look.shape,
    a11y: vm.a11y,
    tip: sessionTipText(s, L, i18n, { ...o, autoResumeInfo: ar }),
 ...
 *   autoResume?: any,   // auto-resume provider (see autoResumeInfo): menu flags and the plan line in row tooltips
 * }} input
 */
function buildSessionList(input) {
  ...
  const o = { now, hints: input.hints || {}, showCost: input.showCost !== false, autoResume: input.autoResume || null };
...
class AgentsViewProvider {
   ...
   *   autoResume?: {                                  // auto-resume (docs/DESIGN.md §12); absent → no auto-resume UI at all
   *     infoFor(session): any,                        //   AutoResumeInfo { available, projectOn, projectName, plan, canNow } | null
   *     now(sessionKey: string): any,                 //   "Continue in background" (may return a promise)
   *     cancel(sessionKey: string): any,              //   cancel the scheduled auto-resume
   *     setProjectFor(session: any, on: boolean): any, //  turn auto-resume on / off for the session's project
   *   },
   * }} opts
   */
  constructor(context, opts = {}) {
    ...
    this.autoResume = opts.autoResume && typeof opts.autoResume === 'object' ? opts.autoResume : null;
    this.autoResumeBusy = new Set(); // sessions whose "Continue in background" is still starting (repeated clicks are ignored)
  ...
  post() {
    ...
    if (this.listInput) {
      const list = buildSessionList({ ...this.listInput, i18n: this.i18n, autoResume: this.autoResume });
    ...
    const { vm } = buildView({
      ...
      autoResumeBusy: this.autoResumeBusy,
    });
  ...
  onMessage(m) {
    ...
    switch (m.type) {
      ...
      case 'more':
        ...
        return;
      // Go to: the button on the session bar, double-click on a list row or an agent row; the command finds the session again by key
      case 'goTo':
        if (this.listed(m.sessionKey)) {
          Promise.resolve(vscode.commands.executeCommand(GOTO_COMMAND, m.sessionKey)).catch((err) => this.log(GOTO_COMMAND + ': ' + ((err && err.message) || err)));
        }
        return;
      ...
      // Auto-resume block in the resume area (current session only): the provider's current state is checked again
      // before acting; the page only says which button was pressed (and, for the project switch, on / off)
      case 'autoResumeNow':
      case 'autoResumeCancel':
      case 'autoResumeProject':
        this.autoResumeAction(m);
        return;
      case 'copyResult': {
      ...
      case 'openTranscript': {
        ...
        if (!isTranscriptFile(file) || !path.isAbsolute(file)) return;
  ...
  // autoResumeNow / autoResumeCancel / autoResumeProject from the page → the provider; the content is reposted once it is done
  autoResumeAction(m) {
    const ar = this.autoResume;
    const [s] = this.lookup(m);
    const info = s ? autoResumeInfo(ar, s) : null;
    if (!info) return;
    const flags = F.autoResumeFlags(info);
    const key = s.key;
    let run = null;
    if (m.type === 'autoResumeNow') {
      if (!flags.bgResumable || this.autoResumeBusy.has(key) || typeof ar.now !== 'function') return;
      this.autoResumeBusy.add(key);
      run = () => ar.now(key);
    } else if (m.type === 'autoResumeCancel') {
      if (!flags.autoResumePending || typeof ar.cancel !== 'function') return;
      run = () => ar.cancel(key);
    } else {
      if (typeof m.on !== 'boolean' || typeof ar.setProjectFor !== 'function') return;
      run = () => ar.setProjectFor(s, m.on);
    }
    this.post(); // hides "Continue in background" while it starts
    Promise.resolve().then(run)
      .catch((err) => this.log(m.type + ': ' + ((err && err.message) || err)))
      .then(() => {
        if (m.type === 'autoResumeNow') this.autoResumeBusy.delete(key);
        this.post();
      });
  }
...
// Transcripts the panel may open: JSONL logs, plus the whole-file JSON sessions older Copilot Chat versions write
function isTranscriptFile(file) {
  return typeof file === 'string' && /\.jsonl?$/i.test(file);
}

module.exports = {
  VIEW_ID, COMPACT_COMMAND, AUTOCOMPACT_COMMAND, REVEAL_COMMAND, COPY_PATH_COMMAND, SHOW_ALL_COMMAND, GOTO_COMMAND,
  ...
  sessionRowVm, sessionTipText, sessionMenuItems, resolveListPosition, autoResumeInfo,

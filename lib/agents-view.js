...
// when: the item appears only if the session carries this flag (compactable / handoff / autoCompact / resumable in data-vscode-context).
 ...
 * @param {{ compactable?: boolean, resumable?: boolean, handoff?: boolean, autoCompact?: boolean }} flags
 */
...
function agentTip(agent, status, lamp, i18n, o) {
  ...
  return lines(
    ...
    unknown ? '' : F.formatTokenBreakdown(tk, i18n),
    counts,
    cost,
...
function bannersVm(s, L, i18n, o) {
  ...
  // The account's latest limit hit (Claude / Copilot / Gemini CLI / Qwen Code), while it still matters: Claude until its reset
  // time, the others (no reset time recorded) for an hour; not when this session's own status already says it
...
function sessionRowVm(s, L, i18n, now, hints) {
  ...
  if (L.lamp === S.LAMP.WORKING && lead && CHURN.has(lead.code) && row.statusText) {
    ...
    // A guessed status keeps its guess mark ("~Working")
    const label = F.lampLabel(S.LAMP.WORKING, i18n);
    const working = lampLib.isGuessStatus(lead) ? F.markGuess(label, i18n) : label;
    const swap = (text, sep) => {
...
function flagsOf(contextValue) {
  ...
  return { compactable: set.has('compactable'), resumable: set.has('resumable'), handoff: cc, autoCompact: cc };
}
...
// Transcripts the panel may open: JSONL logs, plus the whole-file JSON sessions older Copilot Chat and Gemini CLI versions write

...
const KIND_KEY = {
  ...
  copilotSubagent: 'agent.subagent',
};
...
// No reset time known (Copilot): the panel mentions a limit hit for this long afterwards
...
/**
 * One-line status. The guessed approval wait says "may be" in its own wording.
...
function formatStatus(status, agent, i18n, now = Date.now(), opts = {}) {
  return statusText(status, agent, i18n, now, opts);
}
...
/**
 * Explanation of an inferred status (for the tooltip): the guessed approval wait gets its note; '' for a confirmed status.
 */
...
/**
 * "open now" / "not open" for the tooltip.
 * @param {{ live?: boolean }} session
 */
function formatLive(session, i18n) {
  return i18n.t(session && session.live ? 'tip.open' : 'tip.notOpen');
}
...
const WINDOW_SOURCE_KEY = {
  ...
  'copilot-model': 'ctx.src.copilotModel',
};
 ...
 * @param {string} provider claude / copilot
 ...
 * (Claude), otherwise for an hour after the hit (Copilot reports no reset time).
 * @param {(import('./core/status').QuotaHit & { ms: number })|null} lastHit
 * @param {number} [now]
 */
...
// Plan.state / Plan.unavailable values with a sentence in the dictionary (autoresume.state.* / autoresume.unavailable.*)
const AUTORESUME_STATES = Object.freeze(['scheduled', 'self', 'noReset', 'gaveUp', 'running']);
const AUTORESUME_UNAVAILABLE = Object.freeze(['cliNotFound', 'cliTooOld', 'noCwd']);

/**
 * Whether a session gets auto-resume UI: a Claude Code chat (Codex has no official background resume, §12.2) whose
 * AutoResumeInfo from the extension says it is available.
 * @param {any} session
 * @param {any} info AutoResumeInfo (the extension's autoResume.infoFor(session)) or null
 */
function autoResumeApplies(session, info) {
  return !!(session && session.provider === 'claude' && info && typeof info === 'object' && info.available);
}

function autoResumePlan(info) {
  const p = info && info.plan;
  return p && typeof p === 'object' && AUTORESUME_STATES.includes(p.state) ? p : null;
}

/**
 * Menu flags for a session's AutoResumeInfo: bgResumable = "Continue in background" can run now (not while a run is
 * already starting); autoResumePending = an automatic resume is scheduled and can be cancelled.
 * @param {any} info AutoResumeInfo or null
 * @returns {{ bgResumable: boolean, autoResumePending: boolean }}
 */
function autoResumeFlags(info) {
  if (!info || typeof info !== 'object' || !info.available) return { bgResumable: false, autoResumePending: false };
  const plan = autoResumePlan(info);
  const state = plan ? plan.state : null;
  return { bgResumable: !!info.canNow && state !== 'running', autoResumePending: state === 'scheduled' };
}

/**
 * Auto-resume texts for one session: the plan line, the project line and the notes. Nothing changes every second
 * (times are clock times), so it can go into list rows and tooltips.
 * Every state sentence gets the same variables { time, n, max, project }: time is the scheduled time (scheduled) or the
 * usage-limit reset time (self), '' otherwise; n / max are the attempt number and the maximum.
 * @param {{ available?: boolean, projectOn?: boolean, projectName?: string, canNow?: boolean,
 *   plan?: { state: string, atMs: number|null, attempt: number, max: number, unavailable: string|null }|null }|null} info
 *   AutoResumeInfo from the extension (autoResume.infoFor(session))
 * @param {any} i18n
 * @param {number} [now]
 * @returns {{ available: boolean, state: string|null, stateText: string, unavailableText: string, planText: string,
 *   projectOn: boolean, projectText: string, copyNote: string, claudeOnlyNote: string, howNote: string }}
 *   planText = stateText · unavailableText ('' without a plan); everything is empty when info is not available
 */
function formatAutoResume(info, i18n, now = Date.now()) {
  const out = {
    available: false, state: null, stateText: '', unavailableText: '', planText: '',
    projectOn: false, projectText: '', copyNote: '', claudeOnlyNote: '', howNote: '',
  };
  if (!info || typeof info !== 'object' || !info.available) return out;
  const project = String(info.projectName || '');
  out.available = true;
  out.projectOn = !!info.projectOn;
  out.projectText = i18n.t(out.projectOn ? 'autoresume.project.on' : 'autoresume.project.off', { project });
  out.copyNote = i18n.t('autoresume.note.copy');
  out.claudeOnlyNote = i18n.t('autoresume.note.claudeOnly');
  out.howNote = i18n.t('autoresume.note.how');
  const plan = autoResumePlan(info);
  if (!plan) return out;
  const vars = {
    time: fin(plan.atMs) ? i18n.fmtClock(plan.atMs, now) : '',
    n: fin(plan.attempt) ? plan.attempt : '?',
    max: fin(plan.max) ? plan.max : '?',
    project,
  };
  out.state = plan.state;
  out.stateText = i18n.t('autoresume.state.' + plan.state, vars);
  if (AUTORESUME_UNAVAILABLE.includes(plan.unavailable)) out.unavailableText = i18n.t('autoresume.unavailable.' + plan.unavailable, vars);
  out.planText = [out.stateText, out.unavailableText].filter(Boolean).join(SEP);
  return out;
}
 ...
 * o.autoResumeInfo: the session's AutoResumeInfo (autoResume.infoFor(session)); adds an "Auto-resume" row while there is a plan.
...
function formatSessionTooltip(session, i18n, o = {}) {
  ...
  if (autoResumeApplies(session, o.autoResumeInfo)) {
    const ar = formatAutoResume(o.autoResumeInfo, i18n, now);
    if (ar.planText) rows.push([i18n.t('autoresume.section'), ar.planText]);
  }
 ...
 * session provider-<p> lamp-<lamp> [quota] [resumable] [compactable] [live] [bgResumable] [autoResumePending]
 * @param {any} session
 * @param {string} [lamp]
 * @param {any} [autoResume] the session's AutoResumeInfo (autoResume.infoFor(session)); absent → no auto-resume flags
 */
function sessionContextValue(session, lamp, autoResume) {
  ...
  if (autoResumeApplies(session, autoResume)) {
    const f = autoResumeFlags(autoResume);
    if (f.bgResumable) parts.push('bgResumable');
    if (f.autoResumePending) parts.push('autoResumePending');
  }
  return parts.join(' ');
}
...
module.exports = {
  ...
  formatStatus, formatStatusNote, formatProviderNote, formatLive, formatQuotaHit, formatAutoContinue,
  ...
  formatCost, formatCostNote, formatSessionCost, formatAgentCost, formatCredits, copilotCredits, formatToday,
  ...
  AUTORESUME_STATES, AUTORESUME_UNAVAILABLE, autoResumeApplies, autoResumeFlags, formatAutoResume,

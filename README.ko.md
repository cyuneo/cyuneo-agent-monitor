...
- **오류나 사용량 한도 이후에도 이어서 작업합니다.** 한도가 언제 초기화되는지 보여 주고, 다시 이어 갈 프롬프트나 명령을 준비해 두어 복사만 하면 됩니다. 또는 프로젝트에 자동 재개를 켜 두세요: API 오류가 나거나 사용량 한도가 초기화되면 에이전트 모니터가 Claude Code 채팅을 백그라운드에서 이어 가고, 언제 이어 가는지 알림으로 알려 줍니다.
...
- 닫힌 채팅을 백그라운드에서 압축하거나 자동 재개를 쓰려면 Claude Code 명령줄도 필요합니다(자동 재개에는 `--bg`를 지원하는 버전이 필요하며, 2.1.283으로 테스트했습니다). 확장 프로그램은 PATH에서 `claude`를 먼저 찾고, 그다음 설치된 Claude Code 확장 프로그램에서 찾습니다. `agentMonitor.claude.cliPath`를 직접 설정할 수도 있습니다.
...
- 파일을 변경하거나 Claude Code를 실행하는 것은 사용자가 확인한 뒤뿐이며, 자동 재개의 경우에는 사용자가 켜 둔 프로젝트에서만 실행합니다([면책 조항](#면책-조항) 참고).
...
- **사용자가 확인한 경우에만 동작합니다.** 이 확장은 컴퓨터에 있는 세션 기록을 읽습니다. 파일을 변경하거나 Claude Code를 실행하는 것은 사용자가 확인한 뒤뿐입니다: 자동 압축 설정 변경(항목 하나만, 백업 포함), 백그라운드 압축, 인수인계 메모 작성, 백그라운드에서 채팅 이어 가기. 자동 재개는 매번 묻지 않고 Claude Code를 실행하지만, 사용자가 켜 둔 프로젝트에서만 실행합니다.
- **사용량과 비용은 사용자 부담입니다.** 백그라운드 압축, 인수인계 메모, 자동 재개는 사용자의 Claude Code를 실행하며, 요금제의 사용량 한도나 API 요금에 포함됩니다. 표시되는 비용은 공식 가격표 기준 추정치이며 청구서가 아닙니다.

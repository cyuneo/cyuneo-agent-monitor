...
![VS Code 패널에서 동작하는 Agent Monitor 데모 애니메이션입니다. 데이터는 가상의 예시입니다. Claude Code 채팅에서 메인 에이전트와 서브에이전트 두 개가 파일 읽기, 검색, 편집, 테스트 실행을 차례로 진행하고, 토큰과 비용, 컨텍스트 비율이 올라갑니다. 서브에이전트 하나가 테스트를 실행하려고 승인을 기다리는 동안 마젠타색 등이 켜지고, 목록에 있는 그 채팅의 등과 패널 탭의 숫자도 함께 바뀝니다. 승인 후 다시 진행되어 모든 에이전트가 끝나면 초록색 등이 켜집니다. 마지막으로 목록에서 Codex 채팅을 고르면 그 채팅의 단계가 표시됩니다](images/demo.gif)

...
> **프리뷰(0.4.0).** 이상한 점을 발견하면 [GitHub Issues](https://github.com/cyuneo/cyuneo-agent-monitor/issues)에 알려주세요.
...
- **자리를 비워도 알림을 받습니다.** 채팅이 나를 기다리기 시작하면 VS Code나 시스템이 알려 줍니다. 원하면 휴대폰이나 팀 채팅(ntfy, Bark, ServerChan, Feishu, DingTalk, WeCom, Telegram, Discord, Slack)으로 짧은 메시지를 보낼 수도 있고, 오류와 사용 한도도 알려 줍니다.
...
Claude Code와 Codex가 이미 컴퓨터에 기록해 둔 내용만 읽으며 어떤 데이터도 수집하지 않습니다. 푸시 알림(기본값은 꺼짐)을 켜거나 직접 테스트 메시지를 보내지 않는 한 확장 프로그램 자체는 네트워크에 접속하지 않습니다.
...
- 원격 분석이 없고, 푸시 알림을 켜거나 직접 테스트 메시지를 보내지 않는 한 네트워크에 접속하지 않습니다. 대화 내용은 사용자의 컴퓨터에만 남습니다.
...
- **각 서비스의 약관을 지켜 주세요.** Claude Code, Codex 및 관련 서비스, 그리고 설정한 푸시 서비스를 약관에 맞게 사용하는 것은 사용자의 책임입니다.

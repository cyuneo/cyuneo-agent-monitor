...
![下部パネルのエージェントモニタータブ。データは架空のサンプルです。左に選択したClaude Codeチャットのメインエージェント、2つのサブエージェント、ワークフローのエージェント、右に開いているチャットと最近のチャットの一覧](../images/split-view.png)
...
- **チャット一覧。** 開いているチャットと最近のチャットがすべて、それぞれステータスランプ付きで表示されます。**開いている**（Claude Codeがまだチャットを開いている、またはCodexかGitHub Copilot Chatがターンの途中）と**最近**にグループ分けされます。行の高さはターミナルのタブと同じで、選択したチャットはハイライトされます。タイトルが読めるよう、各行にはステータスランプとタイトルだけが表示されます。コンテキストが大きく対応する価値があるときは、タイトルの後ろに小さな印が付きます: **◔** 圧縮を検討、**◕** 早めに対応。行にカーソルを合わせると、ステータスやコンテキストなどの詳細がわかります。
...
ライブ状態を報告するClaude Codeのバージョン（[読み取る内容](#読み取る内容)を参照）とGitHub Copilot Chatでは、「対応が必要」は正確です。ただしCopilot Chatでは最大1分ほど遅れて表示されることがあります。それより古いClaude CodeのバージョンとCodexでは、拡張機能は推測するしかありません。クイックツール（ファイルの読み書き・編集、検索、パッチ）が60秒間結果を返さないと、チャットに**承認待ちの可能性**が表示されます。シェルコマンドなどの長時間コマンドは推測しません。この推測は`agentMonitor.approvalGuess`で変更または無効化できます。

### チャットが動いている場所へ移動

チャットのヘッダーにある**移動**をクリックするか、一覧のチャットまたはそのエージェントをダブルクリックすると、エージェントモニターがそのチャットの動いている場所を表示します。シングルクリックはこれまでどおり、チャットを選択するかエージェントの詳細を表示するだけです。**チャットへ移動**は、チャットの右クリックメニューと**…**メニュー、サイドバーの概要の右クリックメニュー（チャットの行にはインラインボタンもあります）、コマンドパレットにもあります。

| チャットが動いている場所 | 動作 |
| --- | --- |
| Claude Code拡張機能 | Claude Codeでチャットが開きます。タブかサイドバーかは`claudeCode.preferredLocation`の設定に従い、この設定は変更しません |
| Codex拡張機能 | 会話がエディターのタブで開きます |
| GitHub Copilot Chat | チャットが属するワークスペースのウィンドウで、エディターのタブとして開きます |
| VS Codeの統合ターミナルで動くClaude CodeまたはCodex | そのターミナルを表示します |
| 別のVS Codeウィンドウ | そのウィンドウがチャットを開くかターミナルを表示し、前面に出ます。`agentMonitor.shareScanAcrossWindows`がオン（既定）である必要があります。ウィンドウを前面に出せなかったときは、どのウィンドウに切り替えればよいかをメッセージでお知らせします |
| macOSのTerminal.appまたはiTerm2 | そのアプリが前面に出て、チャットのタブが選択されます（下記を参照） |
| ClaudeまたはCodexのデスクトップアプリ、ほかのターミナルアプリ（Warp、Ghostty、WezTerm、kitty、Alacritty、Windows Terminalなど） | まだ対応していません。メッセージでお知らせします |

- **Terminal.appとiTerm2:** 初回は、そのアプリでこのチャットのタブに切り替えるかを、エージェントモニターが先に確認します。**続行**を選ぶと、VS Code（またはお使いのエディター）がそのアプリを操作してよいかをmacOSが確認するので、「許可」を選んでください。確認はアプリごとに1回だけです。許可しなかった場合は、次に試したときに**オートメーション設定を開く**ボタン付きのメッセージが表示されます。システム設定 > プライバシーとセキュリティ > オートメーションで許可してから、もう一度お試しください。あとからそこで変更することもできます。
- **Codex CLI:** これらのチャットには使えるプロセスIDがないため、エージェントモニターはチャットのフォルダーで動いている`codex`のプロセスを探します（どのプロセスがチャットの記録ファイルを開いているかわかる場合は、そのプロセスを使います）。同じフォルダーで複数動いている場合は最も新しく起動したものを選ぶため、別のチャットのこともあります。Windowsではプロセスのフォルダーを読み取れないため、最も新しいものを選びます。
- **移動できないときは**、理由をメッセージでお知らせします。たとえば、チャットがもう動いていない、VS Codeの外のターミナルで動いている、ターミナルやチャットパネルではなく別のツールやエージェントが起動した、そのCopilotチャットのワークスペースを開いているVS Codeウィンドウがない、といった場合です。
- **移動ボタン**（と概要のインラインボタン）は、移動できる見込みが高いときだけ表示されます: まだ開いているClaude Codeのチャット、そして開いているか過去24時間以内に動きがあったCodexとCopilot Chatのチャットです。ダブルクリックとメニューは常に試します。
- **求められたときだけ調べます。** エージェントモニターが実行中のプロセスを一覧するのは、移動を使ったときだけです: macOSとLinuxでは`ps`を1回、WindowsではPowerShellを1回呼び出します。Codex CLIについてはプロセスの作業フォルダーと、どのプロセスがチャットの記録ファイルを開いているかも調べます（macOSでは`lsof`、Linuxでは`/proc`）。そのためにチャットの内容を読むことはなく、どこにも送信しません。ほかのVS Codeウィンドウとは、[読み取る内容](#読み取る内容)で説明している共有フォルダーを通じてやり取りします。
...
- 対応できなかったときにスマートフォンにも知らせてほしい場合は、[スマートフォンやチームのチャットへのプッシュ](#スマートフォンやチームのチャットへのプッシュ)をご覧ください。音でも知らせてほしい場合は[サウンド](#サウンド)を、夜は静かにしておきたい場合は[おやすみ時間](#おやすみ時間)をご覧ください。
...
- **しきい値アラートが出たとき**。使用量、今日のコスト、またはチャットのコンテキストが、決めておいた値を超えたときです（[しきい値アラート](#しきい値アラート)を参照）。イベント名は`usageHigh`、`costDaily`、`contextHigh`です。
- **自動再開がチャットを続行したとき**、コピーとして続行したとき、または続行できなかったとき（[自動再開](#自動再開)を参照）。イベント名は`autoResume`です。

すべて既定でオンです（コストとコンテキストのアラートは、しきい値を設定するまで出ません）。設定メニューの**イベントを選ぶ…**、または`agentMonitor.push.events`で選べます。[おやすみ時間](#おやすみ時間)の間はプッシュしません。エラーを許可した場合は、APIエラーと使用上限への到達だけプッシュします。

**ネットワークアクセス:** ネットワークを使うのはプッシュだけなので、**ネットワークアクセスを許可**（`agentMonitor.network.allow`、既定はオフ）もオンにする必要があります。オフの間はテストメッセージも含めて何も送信されず、設定メニューではプッシュが**一時停止中**と表示されます。オフのままチャネルを追加したり、プッシュをオンにしたり、テストメッセージを送ろうとしたりすると、何がどこに送られるかを説明するダイアログが出て、その**ネットワークアクセスを許可**ボタンを押したときだけオンになります。再びオフにするには**ネットワークアクセスをブロック**（設定メニュー、パネルの**…**メニュー、またはコマンドパレット）を使います。プッシュはオンのまま残り、再び許可しても、その間に起きたことは送信されません。ネットワークアクセスの許可はこのコンピューターだけに適用され、設定の同期で他のコンピューターに引き継がれることはありません。

**送る内容:** プロジェクトフォルダー名と状態です（例:「エージェントへの対応が必要です · my-app」）。使用上限については製品名とリセット時刻です。しきい値アラートについては、何がしきい値を超えたかを送ります。使用量の期間とその割合、今日のコストが予算を超えたこと（金額は送りません）、またはチャットのコンテキストの割合です。自動再開については、何回目の試行かと、続行できなかった場合はその理由です。チャットのプロジェクトで自動再開がオンのときは、そのエラーや使用上限についてのメッセージに、いつ続行するかも書かれます。`agentMonitor.push.includeTitle`をオンにするとチャットのタイトル（とサブエージェント名）が加わりますが、送るのは本当のタイトル（自分で付けたもの、またはエージェントやアプリが付けたもの）だけです。最初のプロンプトから作ったタイトルは決して送りません。拡張機能がプロンプト、コード、ファイルパス、トークン数、コストを付け加えることはありませんが、タイトルとサブエージェント名は書かれたとおりに送るため、ファイル名が含まれることがあります。

**設定方法:** コマンドパレット、またはパネルのタイトルバーの**…**メニューから**エージェントモニター: プッシュ通知…**を実行し、**プッシュのチャネルを追加**を選びます。サービスを選び、プライバシーに関する説明（サービスごとに1回だけ表示）を読み、各項目を入力してからテストメッセージを送ります。トークン、キー、Webhook URLはVS Codeの安全なストレージに保管され、`settings.json`に書かれることはありません。チャネルを編集するときは、秘密情報の欄を空のままにすると保存済みの値を使い、任意の秘密情報（ntfyのアクセストークン、FeishuやDingTalkの署名シークレット）は`-`と入力すると削除できます。秘密情報はコンピューター間で同期されません。設定の同期を使っていると、別のコンピューターで追加したチャネルは、このコンピューターでも設定するまで**このコンピューターでは未設定**と表示されます。チャネルを削除すると、削除したコンピューターの秘密情報だけが消えます（ほかのコンピューターに残ったものが再び使われることはありません）。自前のntfyやBarkのサーバーに`http://`を使えるのは、localhostとプライベートネットワークのアドレスだけです。暗号化されず、別のネットワーク（カフェやホテル）では同じアドレスが他人の機器であることもあるので、自分で管理しているネットワークのサーバーでだけ使ってください。同じメニューで、プッシュがオンかどうか、使用中のチャネル数、ネットワークアクセスが許可されているかを確認でき、テストメッセージの送信、チャネルの編集・オフ・削除、プッシュのオン・オフ、イベントの選択、ネットワークアクセスの許可・ブロックができます。
...
オプション機能で、**既定はオフ**です。`agentMonitor.sound.enabled`をオンにすると、次のようなときにエージェントモニターが短いサウンドを鳴らします。

- **エージェントが対応を待っているとき**（`agentMonitor.sound.needsYou`）
- **エージェントがエラーで停止したとき**（APIエラーや使用上限への到達を含む。`agentMonitor.sound.error`）
- **チャットが完了し、まだ確認していないとき**。つまりランプが明るい緑の**完了（未確認）**に変わったとき（`agentMonitor.sound.done`）
- **しきい値アラートが出たとき**（`agentMonitor.sound.alert`。[しきい値アラート](#しきい値アラート)を参照）

それぞれの設定には`default`、`off`、またはGlass、Ping、Pop、Tink、Submarine、Funk、Hero、Bassoのいずれかを指定できます。`default`では場面ごとに違うサウンドを使います。対応が必要なときはGlass、エラーはBasso、完了はHero、しきい値アラートはFunkです。これらの名前はmacOSのシステムサウンドで、`afplay`で再生します。Linuxでは、システムのサウンドテーマから似たサウンドを`canberra-gtk-play`または`paplay`で再生します（どちらかがインストールされている必要があります）。Windowsでは、Windowsの`Media`フォルダーから似たサウンドを再生します。「対応が必要」のサウンドは[「対応が必要」の通知](#対応が必要の通知)と一緒に鳴るので、`agentMonitor.notifyNeedsYou`もオンにしておく必要があります。いま見ているチャットでは「対応が必要」と「完了」のサウンドは鳴りません。また、完了が推測にすぎないときも「完了」のサウンドは鳴りません。サウンドを鳴らすのは1つのVS Codeウィンドウだけで、いくつものことが同時に起きても鳴るのは1回だけです。すべてのウィンドウを合わせて、最大で3秒に1回です。リモートウィンドウ（SSH、WSL、コンテナー）と[おやすみ時間](#おやすみ時間)の間はサウンドを鳴らしません。

### おやすみ時間

オプション機能で、**既定はオフ**です。`agentMonitor.quietHours.enabled`をオンにすると、決めた時間帯には次のようになります。

- サウンドを鳴らしません。
- システム通知を出しません。メッセージは、次に切り替えたVS Codeウィンドウに表示されるまで待ちます。
- スマートフォンやチームのチャットにプッシュしません。おやすみ時間中に起きたことを後から送ることもありません。

フォーカスのあるVS Codeウィンドウのメッセージはそのまま表示され、パネル、ランプ、バッジ、ステータスバーもいつもどおり更新されます。おやすみ時間中であることは、ステータスバーのツールチップでわかります。

- `agentMonitor.quietHours.start`と`agentMonitor.quietHours.end`は24時間制`HH:MM`の現地時刻で、既定は22:00と08:00です。終了が開始より早い場合は、日付をまたいで続きます。両方が同じ場合、おやすみ時間は始まりません。
- `agentMonitor.quietHours.days`で特定の曜日だけに絞れます: `sun`、`mon`、`tue`、`wed`、`thu`、`fri`、`sat`。空（既定）なら毎日です。基準はおやすみ時間が始まる曜日で、`["fri"]`なら金曜22:00から土曜08:00までがおやすみ時間になります。
- `agentMonitor.quietHours.allowErrors`をオンにすると、APIエラーと使用上限への到達は、おやすみ時間中でもエラーのサウンドが鳴り、プッシュも送ります。上限のリセットとしきい値アラートは静かなままです。

### しきい値アラート

数値が決めておいた値を超えたときにも、エージェントモニターが知らせることができます。知らせ方は「対応が必要」の通知と同じです（フォーカスのあるVS Codeウィンドウにはメッセージ、どのVS Codeウィンドウにもフォーカスがなければシステム通知）。サウンドがオンならしきい値アラートのサウンドが鳴り、プッシュがオンならプッシュも送ります。

| 設定 | 既定値 | 知らせるとき |
| --- | --- | --- |
| `agentMonitor.alerts.usagePercent` | `90` | Codexの5時間または週間の使用量がこの割合に達したとき。Codex専用です。Claude Codeのローカルのログには上限に達したことしか残らず、割合は残りません |
| `agentMonitor.alerts.dailyCost` | `0`（オフ） | 今日の推定API換算コスト（Claude CodeとCodexの合計）がこの金額（米ドル）に達したとき |
| `agentMonitor.alerts.contextPercent` | `0`（オフ） | チャットのメインの会話が自動圧縮ポイントのこの割合に達したとき（Claude CodeとCodex。GitHub Copilot Chatには自動圧縮ポイントがありません） |

- 各アラートは1回だけです。使用量は期間ごとにリセットまで1回、コストは1日1回、コンテキストはチャットごとに次の圧縮まで1回です。値を0にすると、そのアラートはオフになります。
- ウィンドウを開いた時点や設定を変えた時点ですでにしきい値を超えているものは知らせません。
- 1つのアラートを表示するのは1つのウィンドウだけです。
- コストは公開価格による推定値で（[使用量とコスト](#使用量とコスト)を参照）、「今日」はお使いのコンピューターの現地時刻に従います。
- プッシュがオンなら、これらのアラートもプッシュします（[スマートフォンやチームのチャットへのプッシュ](#スマートフォンやチームのチャットへのプッシュ)を参照）。コストのアラートをプッシュするときは、今日のコストが予算を超えたことだけを送り、金額は送りません。
...
コンテキストが20Kトークン以上あるClaude CodeやCodexの各チャットには、**圧縮…**ボタンが付きます。チャット一覧でカーソルを合わせたときのその行にも、エージェントの上にあるチャットヘッダーにもあります。選ぶまでは何も実行されません。
...
- **パーセンテージとウィンドウはClaude CodeとCodex自身の数値をそのまま使います。** パーセンテージはClaude Code自体の計算式を使います: コンテキストトークン数（最新の呼び出しの入力+キャッシュ書き込み+キャッシュ読み取り）÷ コンテキストウィンドウ、四捨五入。ウィンドウはそのセッションのClaude Codeのコスト記録（モデルが1Mウィンドウで実行されているかがわかる）から取得し、それがなければモデルの標準ウィンドウを使います。Codexの場合はCodexのログから取得します。ほかのツールについては[対応ツール](#対応ツール)を参照してください。
...
- **API換算コスト:** 各エージェント、各チャット、そして今日の合計（Claude CodeとCodexのみ）。Claude Codeのチャットの場合、ヘッダーにはClaude Code自身の累計コスト（「Claude Codeの集計」）が表示されます。それ以外はすべてトークン数と公開APIの価格表から推定した値です。サブスクリプションはこの方式で課金されないため、比較用として扱ってください。GitHub Copilot Chatではコストの代わりにCopilot creditsが表示されます（[対応ツール](#対応ツール)を参照）。

### 使用量の履歴

パネルのタイトルバーの**…**メニューまたはコマンドパレットで**エージェントモニター: 使用量の履歴を表示**を実行すると、Claude CodeとCodexの直近30日の使用量を示すページが開きます（GitHub Copilot Chatは含まれません）。

- 1日ごとの推定コストまたはトークン数を棒グラフで表示します。Claude CodeとCodexは積み上げて表示します（**コスト**と**トークン**を切り替えたり、**表で表示**を選んだりできます）。
- 期間の合計、1日平均、使った日数を表示します。
- **モデル別**の内訳を表示します: 入力、出力、キャッシュ読み取り、キャッシュ書き込み、推論、全トークン、推定コスト。

![使用量の履歴ページ。データは架空のサンプルです。30日間の1日ごとの推定コスト（Claude CodeとCodexを積み上げ）、合計、モデル別の内訳](../images/history.png)

しくみ:

- パネルが読むのと同じ記録から、お使いのコンピューター上で計算します（[読み取る内容](#読み取る内容)を参照）。記録を読むのはページが開いている間だけで、バックグラウンドで読み込みます。ページを見ている間は1分ごとに更新されます。
- 結果は拡張機能のストレージにキャッシュされるので、次に開いたときは新しく増えた分だけを読みます。
- コストは、拡張機能のほかの場所と同じく、公開価格によるAPI換算の推定値です。公開価格のないモデルのトークンはコストに含めず、その量をページに表示します。
- 日付はお使いのコンピューターの現地時刻で区切ります。
- 数えるのは、まだコンピューター上に残っているトランスクリプトだけです。Claude Codeは保持期間（`cleanupPeriodDays`、既定は30日）を過ぎた古いトランスクリプトを削除するため、古い日は実際より少なく表示されることがあります。

### 使用上限と再開

- Claude Codeのチャットがセッション、週間、モデルの上限に達したタイミングと、上限がいつリセットされるかを表示します。
- Codexの5時間・週間の使用率を表示します。
- Claude CodeやCodexのチャットまたはエージェントが使用上限、APIエラー、または中断によって停止したとき、**再開プロンプトをコピー…**がすぐ使えるプロンプトやターミナルコマンド（`claude --resume …`、`codex resume …`）をコピーします。コンテキストを読み直すのにかかるコストも見積もります。
- Claude Codeでは、エージェントモニターがチャットを自動で続行することもできます。[自動再開](#自動再開)をご覧ください。

### 自動再開

オプション機能で、**既定はオフ**です。対象はClaude Codeのチャットだけです。プロジェクトでオンにすると、そのプロジェクトのClaude Codeのチャットが停止したとき、次のタイミングでエージェントモニターがバックグラウンドで自動的に続行します。

- **APIエラーの後**（過負荷、タイムアウト、接続切断など）: `agentMonitor.autoResume.errorDelayMinutes`で決めた時間（既定は2分）がたってから続行し、以降は試行のたびに待ち時間が2倍になります（2分、4分、8分）。
- **使用上限の後:** リセット時刻がわかる場合は、上限がリセットされた1分後に続行します。これは`agentMonitor.autoResume.afterLimit`でオフにできます。Claude Code自身が続行する場合（ターミナルのチャットで、Claude Codeの`autoContinueAtUsageLimit`がオンのとき。これがClaude Codeの既定です）はClaude Codeに任せ、パネルにもそう表示されます。

このとき、チャットのフォルダーでローカルのClaude Codeを実行します: `claude --bg --resume <chat> "<resume prompt>"`。再開プロンプトは**再開プロンプトをコピー…**と同じもので、表示言語で書かれます。Claude Codeはウィンドウを開かずに、そのチャットのモデルのままバックグラウンドで作業を続けます。1回ごとの実行は通常のClaude Codeの1ターンで、使用量はプランまたはAPIの請求に反映されます。

- **オンにする方法:** Claude Codeのチャットの詳細で「自動再開」にある**このプロジェクトでオンにする**をクリックするか、コマンドパレットで**自動再開: プロジェクトを選択…**を実行してフォルダーにチェックを入れます。そのフォルダーとサブフォルダーにあるすべてのチャットが対象です。この一覧はこのコンピューターだけに保存され、設定の同期で引き継がれることはなく、ワークスペースの設定で変えることもできません。
- **表示される内容:** チャットの再開が予定されると、その詳細にいつ再開するかが表示され（例:「15:05に自動再開（1/3回目）」）、**キャンセル**と**今すぐ再開**のボタン付きのメッセージも表示されます。エラーや上限についてのプッシュメッセージにも「このプロジェクトは自動再開がオンです: 15:05に続行します（1/3回目）。」と書かれます。その後、チャットが続行したか、コピーとして続行したか、続行できなかったかを、メッセージとプッシュ（`autoResume`イベント）でお知らせします。
- **回数:** 1回の停止につき再開は1回まで、1つのチャットにつき続けて`agentMonitor.autoResume.maxAttempts`回（既定は3回）までです。チャットが1ターンを終えるか、最後の試行から24時間たつと、回数は数え直しになります。停止から24時間以上たったものは再開しません。
- **まだ開いているチャット**（Claude Codeのパネル、ターミナル、またはすでにバックグラウンドで動いているもの）: Claude Codeはコピーをバックグラウンドで続行し、元のチャットはそのまま残します。コピーは一覧に新しいチャットとして表示され、同じ回数の上限に含まれます。
- **バックグラウンドで続行しているチャットの探し方:** ウィンドウはありません。`claude agents`でこうしたチャットの一覧を表示でき、`claude attach <id>`でそのうち1つをターミナルで開けます。`<id>`はチャットのIDの先頭8文字です。
- **今すぐバックグラウンドで続行**は、停止したClaude Codeのチャット（エラー、使用上限、中断、または活動なし）の右クリックまたは**…**メニューと、その詳細の「自動再開」にあります。自動再開がオンかどうかに関係なく、同じ方法ですぐに続行します。自動の試行としては数えず、その停止が後から自動で再開されることもありません。
- **自動再開をキャンセル**は、同じ場所とメッセージにあり、その停止について予定された再開だけをスキップします。チャットが次に停止したときは、また再開が予定されます。
- **Claude Codeがまだ信頼していないフォルダー:** バージョン2.1.283以降のClaude Codeは、信頼の確認を承認したフォルダーでしかチャットをバックグラウンドで続行しません。そのフォルダーをVS CodeのClaude Code拡張機能からしか使ったことがなければ、まだ承認していないかもしれません。その場合は、そのことを伝える**ターミナルを開く**ボタン付きの警告が表示されます。**ターミナルを開く**を押すと、そのフォルダーでターミナルが開き、Claude Codeのコマンドが入力された状態になります。Enterを押し、信頼の確認を承認してから、`/exit`と入力してください。それ以降は、そのフォルダーで自動再開と**今すぐバックグラウンドで続行**が使えます。
- **必要なもの:** `--bg`に対応したバージョンのClaude Codeのコマンドライン（2.1.283でテスト済み）です。バックグラウンド圧縮と同じ方法で、`agentMonitor.claude.cliPath`、PATH、Claude Code拡張機能の順に探します。見つからない場合やバージョンが古すぎる場合は、チャットの詳細にそう表示されます。
...
このページの対象はClaude CodeとCodexのみです。サイズはバックグラウンドで、ページが開いている間だけ、最大10分に1回測定されます。
...
- **GitHub Copilot Chat**: VS Codeに組み込まれたチャットで、エージェントモードとそのサブエージェントを含みます。
- インストールされていないツールはスキップされます。各ツールは設定で個別にオフにできます（[設定](#設定)を参照）。

ツールごとに記録される内容が違うため、拡張機能から見える内容も異なります。

| | GitHub Copilot Chat |
| --- | --- |
| **開いている・作業中・完了** | チャットの記録から読み取るが、最大1分ほど遅れる: VS Codeがチャットを保存するのは約1分に1回のため |
| **対応が必要** | 正確（同じ遅れあり） |
| **トークンとコスト** | チャット全体のトークン（サブエージェント別はなし）。ドル建てのコストの代わりにCopilot credits |
| **コンテキスト** | VS Codeがチャットと一緒に保存するモデル情報のウィンドウ。自動圧縮ポイントなし |

使用量の履歴、今日の合計コスト（とそのアラート）、[保存場所のページ](#チャットの保存場所と移動方法)はClaude CodeとCodexのみが対象です。**圧縮…**、引き継ぎメモ、自動圧縮のしきい値、再開プロンプトもClaude CodeとCodexのチャットだけに表示され、バックグラウンド圧縮は引き続きClaude Code専用です。

### ターミナル版

![同じランプと同じ固定された順序を使うターミナル版](../images/terminal.png)

同じ監視ロジックはターミナルでも動きます。ソースリポジトリには含まれていますが拡張機能パッケージには含まれておらず、Node.js 20以降が必要です（Node.js 20、22、24でテスト済み）:
...
他のオプションには`--provider all|claude|codex|copilot`（既定は`all`）、`--window <minutes>`、`--here`（現在のフォルダーのチャットのみ）、`--today`、`--lang en|zh-cn|zh-tw|ko|ja`、`--no-color`があります。完全な一覧は`--help`で確認してください。
...
すべて読み取り専用で、ユーザー自身が始める例外が1つだけあります: 自動圧縮のしきい値を設定すると、拡張機能がClaude Codeの設定ファイルに`autoCompactWindow`キーを書き込むことがあります（[自動圧縮容量を自分で設定する](#自動圧縮容量を自分で設定する)を参照）。閉じたチャットをバックグラウンドで圧縮するとき、そのチャット記録に要約を追加するのはClaude Code自身です。同じように、チャットをバックグラウンドで続行するとき（[自動再開](#自動再開)を参照）も、その記録を書き継いだり、コピーを書き込んだりするのはClaude Code自身です。
...
| `~/.claude/settings.json`、`<project>/.claude/settings.json`、`<project>/.claude/settings.local.json` | 自動圧縮の設定（`autoCompactWindow`、`autoCompactEnabled`）、`cleanupPeriodDays`、そして自動再開のための`autoContinueAtUsageLimit`（`~/.claude/settings.json`のみ） |
...
| `<User>/workspaceStorage/<workspace>/chatSessions/<session>.jsonl`（または`.json`） | フォルダーやワークスペースを開いたウィンドウのGitHub Copilot Chatのチャット。`<User>`は使用中のVS CodeのUserフォルダーで、たとえばmacOSでは`~/Library/Application Support/Code/User`、Linuxでは`~/.config/Code/User`、Windowsでは`%APPDATA%\Code\User` |
| `<User>/globalStorage/emptyWindowChatSessions/<session>.jsonl`、および`<User>/profiles/<profile>/`以下の同じ場所 | フォルダーを開いていないウィンドウのCopilot Chatのチャット（ほかのプロファイルを含む） |
| `<User>/workspaceStorage/<workspace>/workspace.json` | Copilot Chatのチャットがどのフォルダーに属するか |

上の表のパスは既定の場所です。`CLAUDE_CONFIG_DIR`と`CODEX_HOME`は尊重されます。`agentMonitor.claude.projectsDir`と`agentMonitor.codex.home`で別の場所を指定することもできます。

拡張機能はGitHub Copilot Chatのファイルを読み取るだけで、それらのファイルやそのツールのフォルダーには何も書き込みません。その内容がどこかへ送られることもありません。プッシュをオンにした場合にだけ、[スマートフォンやチームのチャットへのプッシュ](#スマートフォンやチームのチャットへのプッシュ)で説明している短いメッセージが送られます。
...
`agentMonitor.shareScanAcrossWindows`がオンの間（既定）は、ウィンドウが1つだけでも、拡張機能のグローバルストレージにあるフォルダー（`shared-scan`）をウィンドウ間で共有します。そこにはどのウィンドウが記録を読み込むかが書かれ、そのウィンドウはほかのウィンドウのために最新の結果を`shared-scan/snapshot.json`に保存します: チャットのタイトル、プロジェクトフォルダー、ステップ、コスト。このファイルは結果が変わるたびに上書きされ、そのウィンドウが正常に閉じると削除されます。各ウィンドウはそこに、自分のフォルダーと、拡張機能ホストおよびターミナルのプロセスIDを書いた小さな記録も置きます。これにより[移動](#チャットが動いている場所へ移動)で、チャットが動いているウィンドウを見つけられます。別のウィンドウにチャットを開くよう頼む依頼もそこに置く小さなファイルで、そのウィンドウが受け取ると削除します。通知については、すべてのエディターを通じて1つのウィンドウだけが表示するように、待機ごとの小さな目印（ハッシュのみで、チャットの内容は含みません）をシステムの一時ディレクトリにある非公開フォルダーに置きます。目印は1日後に削除されます。プッシュをオンにすると、各チャネルの送信時刻（メッセージの内容は含みません）も同じフォルダー（`push-sent.json`）に記録し、上限がすべてのウィンドウで共通に効くようにします。

[自動再開](#自動再開)は、拡張機能のグローバルストレージに`autoresume.json`というファイルを置きます。続行したチャットのIDとその試行回数・時刻、Claude Codeが始めたコピーのID、そして実行またはキャンセルした停止です。チャットの内容は保存しません。自動再開がオンのプロジェクトの一覧は、VS Codeの拡張機能用ストレージに保存され、このコンピューターだけに置かれます。

[使用量の履歴](#使用量の履歴)のページは、拡張機能のストレージにキャッシュを置きます。読み込んだトランスクリプトごとのファイルパス、サイズ、どこまで読んだか、そして日付・モデル別のトークン数と、同じ応答を二重に数えないためのメッセージIDです。チャットの内容は保存しません。キャッシュが変わるのは、ページが開いている間だけです。

## プライバシー

- **ネットワークアクセスを許可しない限り、拡張機能はネットワークリクエストを行いません**（`agentMonitor.network.allow`、既定はオフ）。オフの間は、プッシュ通知やテストメッセージも含め、すべてのリクエストをコードで拒否します。現在これを使うのはプッシュ通知だけです。テレメトリも、分析も、リモートコンテンツもありません。
...
- **自動再開がClaude Codeを実行するのは、オンにしたプロジェクトの中だけです**（既定はオフ）。このとき拡張機能は、チャットのフォルダーでローカルのClaude Codeを実行します（`claude --bg --resume <session> "<resume prompt>"`）。**今すぐバックグラウンドで続行**も、それを選んだときに、1つのチャットについて同じことをします。Claude Codeは自分で使うときと同じようにAnthropicに接続し、使用量はプランまたはAPIの請求に反映されます。プロジェクトの一覧と試行の記録はこのコンピューターに保存されます（[読み取る内容](#読み取る内容)を参照）。
...
- **移動がプロセスを調べるのは、使ったときだけです。** チャットが動いているターミナルやウィンドウを見つけるために、このコンピューターで実行中のプロセスを一覧します（[チャットが動いている場所へ移動](#チャットが動いている場所へ移動)を参照）。そのためにチャットの内容を読むことはなく、どこにも送信しません。タブを選択するためにTerminal.appやiTerm2を操作するのはユーザーが確認した後だけで、macOSも別に許可を求めます。
...
- **サウンドはこのコンピューター上で再生されます。** システム自身の再生ツール（macOSでは`afplay`、Linuxでは`canberra-gtk-play`または`paplay`、WindowsではPowerShell）を使い、ネットワークには何も送信しません。
- **使用量の履歴はこのコンピューター上で計算され**、拡張機能のストレージにキャッシュされます。どこにも送信しません。
...
すべてのコマンドは**エージェントモニター**カテゴリにあり、それぞれどこから使えるかは下の表のとおりです。特定のチャットに作用するコマンドはその行から実行できます。コマンドパレットから実行すると、どのチャットか尋ねられます（選択中のチャットが一番上に表示されます）。

| コマンド | 場所 | 動作 |
| --- | --- | --- |
| **エージェントモニターを表示** | ステータスバーランプ、コマンドパレット | エージェントモニターパネルを開く |
| **サイドバーに概要を表示** | コマンドパレット | サイドバーで概要ツリーを開く |
| **更新** | パネルのタイトルバー、概要のタイトルバー、コマンドパレット | 今すぐ記録を再読み込み |
| **設定を開く** | パネルのタイトルバーの**…**メニュー、概要のタイトルバー、コマンドパレット | エージェントモニターの設定を開く |
| **すべてのセッションを表示** / **このワークスペースのセッションのみ表示** | パネルのタイトルバー、概要のタイトルバー、コマンドパレット | 範囲を切り替え |
| **確認済みにする** / **すべて確認済みにする** | チャットの右クリックまたは**…** / パネルのタイトルバー、コマンドパレット | 「完了（未確認）」を「完了」にする |
| **完了したエージェントを非表示** / **完了したエージェントを表示** | パネルのタイトルバー、概要のタイトルバー、コマンドパレット | 完了したエージェントを非表示または表示 |
| **チャットへ移動** | チャットヘッダーの**移動**、チャットやエージェントのダブルクリック、チャットの右クリックまたは**…**、概要の右クリックとチャット行、コマンドパレット | チャットが動いている場所を表示する。[チャットが動いている場所へ移動](#チャットが動いている場所へ移動)を参照 |
...
| **再開プロンプトをコピー…** | チャットの右クリックまたは**…**（Claude CodeやCodexのチャットに再開する内容があるとき） | 再開プロンプトまたはターミナルコマンドをコピー |
| **今すぐバックグラウンドで続行** | チャットの右クリックまたは**…**、チャットの詳細の「自動再開」（Claude Codeのチャットが停止したとき） | 今すぐチャットをバックグラウンドで続行する。[自動再開](#自動再開)を参照 |
| **自動再開をキャンセル** | チャットの右クリックまたは**…**、チャットの詳細の「自動再開」、メッセージ（再開が予定されているとき） | この停止について予定された再開をスキップ |
| **自動再開: プロジェクトを選択…** | コマンドパレット | 自動再開を使うプロジェクトを選ぶ |
| **圧縮…** | チャット行、右クリックまたは**…**（コンテキスト20K以上のClaude CodeとCodexのチャット）、チャットヘッダー、コマンドパレット | チャットを圧縮する。[圧縮ボタン](#モデルを選べてコスト見積もり付きの圧縮ボタン)を参照 |
| **引き継ぎメモを書いて新しく始める…** | チャットの右クリックまたは**…**（Claude CodeとCodexのチャット）、圧縮メニュー、コマンドパレット | モデルに`HANDOFF.md`を書かせ、`/clear`と続きの手順を案内 |
| **自動圧縮のしきい値を設定…** | チャットの詳細の「自動圧縮」、チャットの右クリックまたは**…**（Claude CodeとCodexのチャット）、コマンドパレット | [自動圧縮容量を自分で設定する](#自動圧縮容量を自分で設定する)を参照 |
...
| **ネットワークアクセスを許可** / **ネットワークアクセスをブロック** | パネルのタイトルバーの**…**メニュー、コマンドパレット | ネットワークアクセスをオン・オフします。[プライバシー](#プライバシー)を参照 |
| **ネットワークアクセスの切り替え** | 「キーボード ショートカット」で自分で割り当てたショートカットからのみ | ネットワークアクセスがオフならオンに、オンならオフにします |
| **使用量の履歴を表示** | パネルのタイトルバーの**…**メニュー、コマンドパレット | [使用量の履歴](#使用量の履歴)を参照 |
...
| `agentMonitor.claude.cliPath` | `""` | Claude Codeのコマンドライン。バックグラウンド圧縮と自動再開に使用（空欄ならPATH上の`claude`、次にClaude Code拡張機能内を探す） |
| `agentMonitor.codex.enabled` | `true` | Codexの記録を読み込む |
| `agentMonitor.codex.home` | `""` | Codexのフォルダー（空欄なら`$CODEX_HOME`または`~/.codex`） |
| `agentMonitor.copilot.enabled` | `true` | GitHub Copilot Chatのセッションを読み込む（VS Code組み込みのチャット） |
...
| `agentMonitor.sound.enabled` | `false` | サウンドを鳴らす。[サウンド](#サウンド)を参照 |
| `agentMonitor.sound.needsYou` | `default` | エージェントが対応を待っているときのサウンド: `default`、`off`、または`Glass`、`Ping`、`Pop`、`Tink`、`Submarine`、`Funk`、`Hero`、`Basso` |
| `agentMonitor.sound.error` | `default` | エージェントがエラーや使用上限で停止したときのサウンド（選択肢は同じ） |
| `agentMonitor.sound.done` | `default` | チャットが完了し、まだ確認していないときのサウンド（選択肢は同じ） |
| `agentMonitor.sound.alert` | `default` | しきい値アラートのサウンド（選択肢は同じ） |
| `agentMonitor.quietHours.enabled` | `false` | おやすみ時間: サウンド、システム通知、プッシュなし。[おやすみ時間](#おやすみ時間)を参照 |
| `agentMonitor.quietHours.start` | `"22:00"` | おやすみ時間の開始時刻（現地時刻、24時間制`HH:MM`） |
| `agentMonitor.quietHours.end` | `"08:00"` | おやすみ時間の終了時刻。開始より早ければ翌日 |
| `agentMonitor.quietHours.days` | `[]` | おやすみ時間が始まる曜日: `sun` … `sat`。空なら毎日 |
| `agentMonitor.quietHours.allowErrors` | `false` | おやすみ時間中も、APIエラーと使用上限への到達ではエラーのサウンドを鳴らし、プッシュする |
| `agentMonitor.alerts.usagePercent` | `90` | Codexの5時間または週間の使用量がこの割合に達したら知らせる（0: オフ）。[しきい値アラート](#しきい値アラート)を参照 |
| `agentMonitor.alerts.dailyCost` | `0` | 今日の推定コストがこの金額（米ドル）に達したら知らせる（0: オフ） |
| `agentMonitor.alerts.contextPercent` | `0` | チャットのメインの会話が自動圧縮ポイントのこの割合に達したら知らせる（0: オフ） |
| `agentMonitor.autoResume.maxAttempts` | `3` | 1つのチャットを続けて自動再開できる回数（1〜10）。チャットが1ターンを終えると回数は数え直しになる |
| `agentMonitor.autoResume.errorDelayMinutes` | `2` | APIエラーの後、再開するまで待つ時間（分、1〜60）。以降は試行のたびに待ち時間が2倍になる |
| `agentMonitor.autoResume.afterLimit` | `true` | リセット時刻がわかる場合は、使用上限がリセットされた1分後にも再開する |
| `agentMonitor.network.allow` | `false` | ネットワークリクエストを許可。オフの間、拡張機能はネットワークリクエストを一切行わない。現在これを使うのはプッシュ通知だけ。このコンピューターだけに適用（設定の同期では同期されない） |
| `agentMonitor.push.enabled` | `false` | スマートフォンやチームのチャットへのプッシュ。[スマートフォンやチームのチャットへのプッシュ](#スマートフォンやチームのチャットへのプッシュ)を参照 |
| `agentMonitor.push.events` | すべてオン | プッシュするイベント: `needsYou`、`error`、`limitHit`、`limitReset`、`usageHigh`、`costDaily`、`contextHigh`、`autoResume` |
...
プッシュの設定と`agentMonitor.network.allow`はユーザー設定からのみ読み込むため、ワークスペースの設定でプッシュやネットワークアクセスをオンにしたり送信先を変えたりすることはできません。
...
- **記録形式が変わる可能性があります。** 3つのツールの記録形式はいずれも内部形式であり、アップデートのたびに変わる可能性があります。拡張機能が読み取れない行はスキップされます。
- **ステップの表示が遅れることがあります。** 記録はモデル呼び出しのたびに書き込まれるため、長い思考の途中では最後に書き込まれたステップが表示されます。
- **「対応が必要」はアプリが状態を報告する場合にのみ正確です。** つまり、`~/.claude/sessions`でライブ状態を報告するClaude CodeのバージョンとGitHub Copilot Chatに限られます。それより古いClaude CodeのバージョンとCodexは上記の推測に頼っており、Codexは承認やエラーについてディスクにほとんど書き込みません。
- **GitHub Copilot Chatは最大1分ほど遅れることがあります。** VS Codeがチャットをディスクに保存するのは約1分に1回なので、新しいステップ、終わった回答、あなたを待っているプロンプトの表示が最大1分遅れることがあります。
- **コストは推定値です。** ツールチップに表示された日付時点の公開価格を使用するため、価格は変動する可能性があります。Codexのレビューモデルのように、公開価格のないモデルもあります。GitHub Copilot Chatではコストではなく、Copilot creditsが表示されます。
- **一部の機能はClaude CodeとCodexのみが対象です。** 使用量の履歴、今日の合計コスト（と1日のコストのアラート）、保存場所のページにはGitHub Copilot Chatは含まれません。また、Copilot Chatには自動圧縮ポイントがありません。**圧縮…**、引き継ぎメモ、自動圧縮のしきい値、再開プロンプトもClaude CodeとCodexのチャットだけが対象です。
- **Claude Codeの使用率は表示されません。** Claude Codeはこれをローカルファイルに保存しないため、Claudeについては上限到達とリセット時刻のみが表示され、使用量のしきい値アラートもCodex専用です。
- **タブ追跡には抜けがあります。** Claude Codeはタブのタイトルで、Codexは会話IDで追跡します。サイドバービュー（エディタータブではなく）に表示されているチャットは検出できません。
- **自動再開はClaude Code専用です。** Codexにはチャットをバックグラウンドで続行する公式の方法がないため、Codexでは使えません。Claude Codeがバックグラウンドで続行しているチャットは**移動**では表示できません。ターミナルで`claude attach <id>`を使ってください。
...
- **通知から直接承認したり答えたりはできません。** エージェントモニターは記録を読むだけです。通知から答えるには、Claude CodeやCodexのhooksにつなぐか、リモートで操作する必要があります。そのチャットに切り替えて答えてください。
- **システム通知をクリックしても、そのチャットへは移動できません。** システム通知はシステムが単独で表示するもので、拡張機能を呼び出せないためです。代わりにパネルの**移動**を使ってください。VS Codeのメッセージの**表示**はパネルでそのチャットを選択し、**移動**はチャットがどのウィンドウにあっても、それが動いている場所へ連れて行きます。
- **移動はまだすべての場所には対応していません。** ClaudeとCodexのデスクトップアプリのチャット、そしてTerminal.appとiTerm2以外のターミナルアプリ（Warp、Ghostty、WezTerm、kitty、Alacritty、Windows Terminalなど）のチャットには、まだ対応していません。Codex CLIは、同じフォルダーで複数動いている場合、最も新しく起動したものを選びます。別のVS Codeウィンドウのチャットへ移動できるのは、`agentMonitor.shareScanAcrossWindows`がオンのときだけです。
- **サウンドはmacOSでしか試していません。** Linuxでは`canberra-gtk-play`または`paplay`で、WindowsではPowerShellでWindowsの`Media`フォルダーのサウンドを再生しますが、どちらもまだ試していません。

...
- **チャット一覧。** 開いているチャットと最近のチャットがすべて、それぞれステータスランプ付きで表示されます。**開いている**（Claude Codeがまだチャットを開いている、CodexかGitHub Copilot Chatがターンの途中、Qwen Codeのプロセスがまだ動いている、またはGemini CLIが最近チャットに書き込んだと推測される）と**最近**にグループ分けされます。行の高さはターミナルのタブと同じで、選択したチャットはハイライトされます。タイトルが読めるよう、各行にはステータスランプとタイトルだけが表示されます。コンテキストが大きく対応する価値があるときは、タイトルの後ろに小さな印が付きます: **◔** 圧縮を検討、**◕** 早めに対応。行にカーソルを合わせると、ステータスやコンテキストなどの詳細がわかります。
...
ライブ状態を報告するClaude Codeのバージョン（[読み取る内容](#読み取る内容)を参照）とGitHub Copilot Chatでは、「対応が必要」は正確です。ただしCopilot Chatでは最大1分ほど遅れて表示されることがあります。それより古いClaude Codeのバージョン、Codex、Gemini CLI、Qwen Codeでは、拡張機能は推測するしかありません。クイックツール（ファイルの読み書き・編集、検索、パッチ）が60秒間結果を返さないと、チャットに**承認待ちの可能性**が表示されます。シェルコマンドなどの長時間コマンドは推測しません。この推測は`agentMonitor.approvalGuess`で変更または無効化できます。
...
| VS Codeの統合ターミナルで動くClaude Code、Codex、Gemini CLI、Qwen Code | そのターミナルを表示します |
...
- **Codex CLIとGemini CLI:** これらのチャットには使えるプロセスIDがないため、エージェントモニターはチャットのフォルダーで動いている`codex`または`gemini`のプロセスを探します（Codexでは、どのプロセスがチャットの記録ファイルを開いているかわかる場合は、そのプロセスを使います）。同じフォルダーで複数動いている場合は最も新しく起動したものを選ぶため、別のチャットのこともあります。Windowsではプロセスのフォルダーを読み取れないため、最も新しいものを選びます。
...
- **移動ボタン**（と概要のインラインボタン）は、移動できる見込みが高いときだけ表示されます: まだ開いているClaude CodeとQwen Codeのチャット、そして開いているか過去24時間以内に動きがあったCodex、Gemini CLI、Copilot Chatのチャットです。ダブルクリックとメニューは常に試します。
- **求められたときだけ調べます。** エージェントモニターが実行中のプロセスを一覧するのは、移動を使ったときだけです: macOSとLinuxでは`ps`を1回、WindowsではPowerShellを1回呼び出します。Codex CLIとGemini CLIについてはプロセスの作業フォルダーも、Codexについてはどのプロセスがチャットの記録ファイルを開いているかも調べます（macOSでは`lsof`、Linuxでは`/proc`）。そのためにチャットの内容を読むことはなく、どこにも送信しません。ほかのVS Codeウィンドウとは、[読み取る内容](#読み取る内容)で説明している共有フォルダーを通じてやり取りします。
...
| `agentMonitor.alerts.contextPercent` | `0`（オフ） | チャットのメインの会話が自動圧縮ポイントのこの割合に達したとき（Claude CodeとCodex。他のツールには自動圧縮ポイントがありません） |
...
パネルのタイトルバーの**…**メニューまたはコマンドパレットで**エージェントモニター: 使用量の履歴を表示**を実行すると、Claude CodeとCodexの直近30日の使用量を示すページが開きます（GitHub Copilot Chat、Gemini CLI、Qwen Codeは含まれません）。
...
- **Gemini CLI**（プレビュー）: サブエージェントを含みます。
- **Qwen Code**（プレビュー）: サブエージェントを含みます。
- インストールされていないツールはスキップされます。各ツールは設定で個別にオフにできます（[設定](#設定)を参照）。

> **Gemini CLIとQwen Codeはプレビューです。** 公開されている記録形式をもとに作っており、まだ実際のセッションでは確認していません。チャットの表示がおかしい場合は、[GitHub Issues](https://github.com/cyuneo/cyuneo-agent-monitor/issues)で報告してください。

ツールごとに記録される内容が違うため、拡張機能から見える内容も異なります。

| | GitHub Copilot Chat | Gemini CLI（プレビュー） | Qwen Code（プレビュー） |
| --- | --- | --- | --- |
| **開いている・作業中・完了** | チャットの記録から読み取るが、最大1分ほど遅れる: VS Codeがチャットを保存するのは約1分に1回のため | チャットが最後に書き込まれた時刻から推測するため、完了に見えたチャットが作業中に戻ることがある。ツール名はツールが終わってから表示される | チャットが開いているかどうかは正確: Qwen CodeのプロセスIDから判断 |
| **対応が必要** | 正確（同じ遅れあり） | 推測 | 推測 |
| **トークンとコスト** | チャット全体のトークン（サブエージェント別はなし）。ドル建てのコストの代わりにCopilot credits | トークンと推定コスト | トークンと、Alibaba Cloud国際版の公開価格による推定コスト。無料のOAuthモデルはトークンのみ |
| **コンテキスト** | VS Codeがチャットと一緒に保存するモデル情報のウィンドウ。自動圧縮ポイントなし | モデルの標準ウィンドウ。自動圧縮ポイントなし | Qwen Codeの記録にあるウィンドウ。自動圧縮ポイントなし |
...
他のオプションには`--provider all|claude|codex|copilot|gemini|qwen`（既定は`all`）、`--window <minutes>`、`--here`（現在のフォルダーのチャットのみ）、`--today`、`--lang en|zh-cn|zh-tw|ko|ja`、`--no-color`があります。完全な一覧は`--help`で確認してください。
...
| `~/.gemini/tmp/<project>/chats/session-*.jsonl`（または`.json`）と`~/.gemini/tmp/<project>/chats/<session>/…` | Gemini CLIのチャットとそのサブエージェント。Gemini CLIがmacOSのサンドボックスで動いているときは、`~/.cache/.gemini`以下の同じファイル。`agentMonitor.gemini.home`を設定していなければ、ここも読みます |
| `~/.gemini/tmp/<project>/.project_root`、`~/.gemini/projects.json` | Gemini CLIのチャットがどのフォルダーに属するか |
| `~/.qwen/projects/<project>/chats/<session>.jsonl` | Qwen Codeのチャット（サブエージェントを含む） |
| `~/.qwen/projects/<project>/chats/<session>.runtime.json` | チャットがまだ開いているかを判断するためのQwen CodeのプロセスID。拡張機能はそのプロセスがまだ動いているかどうかだけを確認します |

上の表のパスは既定の場所です。`CLAUDE_CONFIG_DIR`と`CODEX_HOME`は尊重されます。`GEMINI_CLI_HOME`が設定されていれば、Gemini CLIのフォルダーを`~`ではなく`$GEMINI_CLI_HOME`の下で探します。Qwen Codeでは、`~/.qwen`の代わりに`QWEN_RUNTIME_DIR`、なければ`QWEN_HOME`を使います。`agentMonitor.claude.projectsDir`、`agentMonitor.codex.home`、`agentMonitor.gemini.home`、`agentMonitor.qwen.home`で別の場所を指定することもできます。

拡張機能はGitHub Copilot Chat、Gemini CLI、Qwen Codeのファイルを読み取るだけで、それらのファイルや各ツールのフォルダーには何も書き込みません。その内容がどこかへ送られることもありません。プッシュをオンにした場合にだけ、[スマートフォンやチームのチャットへのプッシュ](#スマートフォンやチームのチャットへのプッシュ)で説明している短いメッセージが送られます。
...
| `agentMonitor.gemini.enabled` | `true` | Gemini CLIの記録を読み込む（プレビュー） |
| `agentMonitor.gemini.home` | `""` | Gemini CLIのフォルダー（空欄なら`$GEMINI_CLI_HOME/.gemini`または`~/.gemini`と、macOSのサンドボックス用フォルダー`$GEMINI_CLI_HOME/.cache/.gemini`または`~/.cache/.gemini`） |
| `agentMonitor.qwen.enabled` | `true` | Qwen Codeの記録を読み込む（プレビュー） |
| `agentMonitor.qwen.home` | `""` | Qwen Codeのフォルダー（空欄なら`$QWEN_RUNTIME_DIR`、なければ`$QWEN_HOME`、なければ`~/.qwen`） |
...
| `agentMonitor.push.events` | すべてオン | プッシュするイベント: `needsYou`、`error`、`limitHit`、`limitReset`、`usageHigh`、`costDaily`、`contextHigh` |
...
- **記録形式が変わる可能性があります。** 5つのツールの記録形式はいずれも内部形式であり、アップデートのたびに変わる可能性があります。拡張機能が読み取れない行はスキップされます。
- **Gemini CLIとQwen Codeへの対応はプレビューです。** 公開されている記録形式をもとに作っており、まだ実際のセッションでは確認していません。チャットの表示がおかしい場合は、[GitHub Issues](https://github.com/cyuneo/cyuneo-agent-monitor/issues)でお知らせください。
- **ステップの表示が遅れることがあります。** 記録はモデル呼び出しのたびに書き込まれるため、長い思考の途中では最後に書き込まれたステップが表示されます。
- **「対応が必要」はアプリが状態を報告する場合にのみ正確です。** つまり、`~/.claude/sessions`でライブ状態を報告するClaude CodeのバージョンとGitHub Copilot Chatに限られます。それより古いClaude Codeのバージョン、Codex、Gemini CLI、Qwen Codeは上記の推測に頼っており、Codexは承認やエラーについてディスクにほとんど書き込みません。Gemini CLIは実行中のセッションの一覧も残さないため、チャットが作業中か完了したかも推測です。
- **GitHub Copilot Chatは最大1分ほど遅れることがあります。** VS Codeがチャットをディスクに保存するのは約1分に1回なので、新しいステップ、終わった回答、あなたを待っているプロンプトの表示が最大1分遅れることがあります。
- **コストは推定値です。** ツールチップに表示された日付時点の公開価格を使用するため、価格は変動する可能性があります。Codexのレビューモデルのように、公開価格のないモデルもあります。GitHub Copilot Chatではコストではなく、Copilot creditsが表示されます。Qwen CodeのコストはAlibaba Cloud国際版の公開価格にもとづきます（ほかのリージョンは価格が異なります）。記録からはキャッシュの種類がわからないため、キャッシュヒットは明示的キャッシュの価格で計算します。無料のOAuthモデルには価格がないため、トークンのみを表示します。
- **一部の機能はClaude CodeとCodexのみが対象です。** 使用量の履歴、今日の合計コスト（と1日のコストのアラート）、保存場所のページにはGitHub Copilot Chat、Gemini CLI、Qwen Codeは含まれません。また、これら3つのツールには自動圧縮ポイントがありません。**圧縮…**、引き継ぎメモ、自動圧縮のしきい値、再開プロンプトもClaude CodeとCodexのチャットだけが対象です。
...
- **移動はまだすべての場所には対応していません。** ClaudeとCodexのデスクトップアプリのチャット、そしてTerminal.appとiTerm2以外のターミナルアプリ（Warp、Ghostty、WezTerm、kitty、Alacritty、Windows Terminalなど）のチャットには、まだ対応していません。Codex CLIとGemini CLIは、同じフォルダーで複数動いている場合、最も新しく起動したものを選びます。別のVS Codeウィンドウのチャットへ移動できるのは、`agentMonitor.shareScanAcrossWindows`がオンのときだけです。

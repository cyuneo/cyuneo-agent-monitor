...
- **자동 재개**가 채팅을 이어 갔거나, 사본으로 이어 갔거나, 이어 가지 못했을 때(이벤트 이름은 `autoResume`, [자동 재개](#자동-재개) 참고).
...
- Claude Code의 경우 에이전트 모니터가 채팅을 알아서 이어 갈 수도 있습니다. [자동 재개](#자동-재개)를 참고하세요.

### 자동 재개

선택 기능이며 **기본값은 꺼짐**이고, Claude Code 채팅에서만 작동합니다. 프로젝트에 켜 두면, 그 안의 Claude Code 채팅이 멈췄을 때 에이전트 모니터가 알아서 백그라운드에서 이어 갑니다.

- **API 오류가 난 뒤**(과부하, 시간 초과, 연결 끊김 등): `agentMonitor.autoResume.errorDelayMinutes`(기본 2분)가 지난 뒤 이어 가며, 그 뒤로는 시도할 때마다 두 배씩 기다립니다(2분, 4분, 8분).
- **사용 한도에 걸린 뒤:** 초기화 시각을 알 수 있으면 한도가 초기화되고 1분 뒤에 이어 갑니다. `agentMonitor.autoResume.afterLimit`으로 끌 수 있습니다. Claude Code가 스스로 계속하는 경우(터미널의 채팅이고, Claude Code의 `autoContinueAtUsageLimit`이 기본값대로 켜져 있을 때)에는 Claude Code에 맡기며, 패널에 그렇게 표시됩니다.

채팅의 폴더에서 로컬 Claude Code를 실행합니다: `claude --bg --resume <chat> "<resume prompt>"`. 재개 프롬프트는 **재개 프롬프트 복사…**와 같은 것이며, 표시 언어로 작성됩니다. Claude Code는 창을 열지 않고 백그라운드에서, 그 채팅의 모델 그대로 이어서 작업합니다. 실행할 때마다 평범한 Claude Code 턴 한 번이며, 요금제나 API 청구서에 반영됩니다.

- **켜는 방법:** Claude Code 채팅의 **자세히**에서 자동 재개 아래의 **이 프로젝트에 켜기**를 클릭하거나, 명령 팔레트에서 **자동 재개: 프로젝트 선택…**을 실행해 폴더를 선택하세요. 그 폴더와 하위 폴더의 모든 채팅에 적용됩니다. 목록은 이 컴퓨터에만 보관됩니다. 설정 동기화로 옮겨지지 않으며, 작업 영역 설정으로 바꿀 수도 없습니다.
- **표시되는 내용:** 채팅의 재개가 예정되면 **자세히**에 언제 재개하는지 표시되고(예: "오후 3:05에 자동 재개 (시도 1/3)"), **취소**와 **지금 재개** 버튼이 있는 메시지가 뜹니다. 오류나 한도에 대한 푸시 메시지에도 같은 내용이 들어갑니다: "이 프로젝트는 자동 재개가 켜져 있습니다: 오후 3:05에 계속합니다 (시도 1/3)." 그 뒤에는 메시지와 푸시(`autoResume` 이벤트)로 채팅이 이어졌는지, 사본으로 이어졌는지, 이어 가지 못했는지 알려 줍니다.
- **재개 횟수:** 한 번의 중단은 최대 한 번만 재개하며, 한 채팅은 연속으로 최대 `agentMonitor.autoResume.maxAttempts`번(기본 3번)까지 재개합니다. 채팅이 한 턴을 마치거나 마지막 시도 후 24시간이 지나면 횟수를 처음부터 다시 셉니다. 중단된 지 24시간이 넘었으면 재개하지 않습니다.
- **아직 열려 있는 채팅**(Claude Code 패널, 터미널, 또는 이미 백그라운드에서 실행 중): Claude Code가 사본을 만들어 백그라운드에서 계속하고, 원래 채팅은 그대로 둡니다. 사본은 목록에 새 채팅으로 나타나며, 같은 횟수 제한에 포함됩니다.
- **백그라운드에서 이어지는 채팅 찾기:** 이런 채팅에는 창이 없습니다. `claude agents`로 이런 채팅의 목록을 볼 수 있고, `claude attach <id>`로 그중 하나를 터미널에서 열 수 있습니다. id는 채팅 ID의 앞 8자입니다.
- **지금 백그라운드에서 계속**은 멈춘 Claude Code 채팅(오류, 사용 한도, 중단, 또는 활동 없음)의 우클릭 메뉴나 **…** 메뉴, 그리고 **자세히**의 자동 재개 아래에 있으며, 자동 재개가 켜져 있든 아니든 같은 방식으로 바로 이어 갑니다. 자동 시도 횟수에는 들어가지 않으며, 그 중단은 다시 자동으로 재개하지 않습니다.
- **자동 재개 취소**는 같은 위치나 메시지에 있으며, 그 중단에 예정된 재개만 건너뜁니다. 채팅이 다음에 멈추면 다시 재개를 예약합니다.
- **Claude Code가 아직 신뢰하지 않는 폴더:** 버전 2.1.283부터 Claude Code는 신뢰 확인을 수락한 폴더에서만 채팅을 백그라운드에서 이어 갑니다. 어떤 폴더를 VS Code의 Claude Code 확장 프로그램으로만 사용했다면 아직 수락하지 않았을 수 있습니다. 그러면 경고에 그 사실이 표시되고 **터미널 열기**가 나오며, 이를 누르면 그 폴더에서 Claude Code 명령이 입력된 터미널이 열립니다. Enter를 누르고 신뢰 확인을 수락한 뒤 `/exit`를 입력하세요. 그다음부터 그 폴더에서 자동 재개와 **지금 백그라운드에서 계속**을 쓸 수 있습니다.
- **필요한 것:** `--bg`를 지원하는 버전의 Claude Code 명령줄(2.1.283으로 테스트). 백그라운드 압축과 같은 방법으로 찾습니다: `agentMonitor.claude.cliPath`, 그다음 PATH, 그다음 Claude Code 확장 프로그램. 없거나 버전이 너무 오래되었으면 채팅의 **자세히**에 그렇게 표시됩니다.
...
모든 것이 읽기 전용이며, 사용자가 직접 시작하는 예외가 하나 있습니다: 자동 압축 임계값을 설정하면 확장 프로그램이 Claude Code 설정 파일에 `autoCompactWindow` 키를 쓸 수 있습니다([자동 압축 용량 직접 설정하기](#자동-압축-용량-직접-설정하기) 참고). 닫힌 채팅을 백그라운드에서 압축할 때, 그 채팅 기록에 요약을 추가하는 것은 Claude Code 자신입니다. 마찬가지로 채팅을 백그라운드에서 이어 갈 때([자동 재개](#자동-재개) 참고) 그 기록을 이어 쓰거나 사본을 쓰는 것도 Claude Code 자신입니다.
...
| `~/.claude/settings.json`, `<project>/.claude/settings.json`, `<project>/.claude/settings.local.json` | 자동 압축 설정(`autoCompactWindow`, `autoCompactEnabled`), `cleanupPeriodDays`, 그리고 자동 재개용 `autoContinueAtUsageLimit`(`~/.claude/settings.json`만) |
...
[자동 재개](#자동-재개)는 확장 프로그램의 전역 저장소에 `autoresume.json` 파일을 둡니다. 이어 간 채팅의 ID와 그 시도 횟수 및 시각, Claude Code가 시작한 사본의 ID, 그리고 재개를 실행했거나 취소한 중단이 들어 있습니다. 채팅 내용은 저장하지 않습니다. 자동 재개가 켜진 프로젝트 목록은 VS Code의 확장 프로그램용 저장소에 보관되며, 이 컴퓨터에만 있습니다.
...
- **자동 재개는 사용자가 켜 둔 프로젝트에서만 Claude Code를 실행합니다**(기본값은 꺼짐). 이때 확장 프로그램은 채팅의 폴더에서 로컬 Claude Code를 실행합니다(`claude --bg --resume <session> "<resume prompt>"`). **지금 백그라운드에서 계속**은 사용자가 선택했을 때 채팅 하나에 대해 같은 일을 합니다. Claude Code는 사용자가 직접 사용할 때와 마찬가지로 Anthropic에 연결되며, 사용량은 요금제나 API 청구서에 반영됩니다. 프로젝트 목록과 시도 기록은 이 컴퓨터에 보관됩니다([읽는 내용](#읽는-내용) 참고).
...
| **지금 백그라운드에서 계속** | 채팅 우클릭 또는 **…**, 채팅 자세히 안의 자동 재개(Claude Code 채팅이 멈췄을 때) | 지금 채팅을 백그라운드에서 이어 가기; [자동 재개](#자동-재개) 참고 |
| **자동 재개 취소** | 채팅 우클릭 또는 **…**, 채팅 자세히 안의 자동 재개, 메시지(재개가 예정되어 있을 때) | 이번 중단에 예정된 재개 건너뛰기 |
| **자동 재개: 프로젝트 선택…** | 명령 팔레트 | 자동 재개를 켤 프로젝트 선택 |
...
| `agentMonitor.claude.cliPath` | `""` | Claude Code 명령줄, 백그라운드 압축과 자동 재개에 사용(비워 두면 PATH에서 `claude`를 찾고, 그다음 Claude Code 확장 프로그램에서 찾음) |
...
| `agentMonitor.autoResume.maxAttempts` | `3` | 한 채팅을 연속으로 자동 재개할 수 있는 횟수(1–10). 채팅이 한 턴을 마치면 횟수를 처음부터 다시 셈 |
| `agentMonitor.autoResume.errorDelayMinutes` | `2` | API 오류가 난 뒤 재개하기 전까지 기다리는 시간(분)(1–60). 이후 시도할 때마다 두 배로 기다림 |
| `agentMonitor.autoResume.afterLimit` | `true` | 초기화 시각을 알 수 있으면 사용 한도가 초기화되고 1분 뒤에도 재개 |
...
| `agentMonitor.push.events` | 모두 켜짐 | 푸시할 이벤트: `needsYou`, `error`, `limitHit`, `limitReset`, `usageHigh`, `costDaily`, `contextHigh`, `autoResume` |
...
- **자동 재개는 Claude Code 전용입니다.** Codex에는 채팅을 백그라운드에서 이어 가는 공식 방법이 없어 Codex에는 제공하지 않습니다. Claude Code가 백그라운드에서 이어 가는 채팅은 **이동**으로 띄울 수 없으니, 터미널에서 `claude attach <id>`를 사용하세요.

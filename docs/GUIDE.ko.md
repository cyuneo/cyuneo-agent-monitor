...
- 응답하지 못했을 때 휴대폰으로도 메시지를 받으려면 [휴대폰이나 팀 채팅으로 푸시](#휴대폰이나-팀-채팅으로-푸시)를 참고하세요.
...
`agentMonitor.shareScanAcrossWindows`가 켜져 있는 동안(기본값)에는 창이 하나만 열려 있어도 창들이 확장 프로그램의 전역 저장소에 있는 폴더(`shared-scan`)를 함께 사용합니다. 이 폴더에는 어느 창이 기록을 읽는지가 기록되며, 그 창은 다른 창을 위해 최신 결과를 `shared-scan/snapshot.json`에 저장합니다: 채팅 제목, 프로젝트 폴더, 단계, 비용. 이 파일은 결과가 바뀔 때마다 덮어쓰며, 그 창이 정상적으로 닫히면 삭제됩니다. 알림의 경우, 모든 편집기를 통틀어 한 창에서만 알림이 표시되도록 대기마다 작은 표시 파일(해시만 있고 채팅 내용은 없음)을 시스템 임시 디렉터리의 비공개 폴더에 남깁니다. 표시 파일은 하루 뒤 삭제됩니다. 푸시를 켜면 각 푸시 채널의 전송 시각(메시지 내용은 없음)도 같은 폴더(`push-sent.json`)에 보관해, 한도가 모든 창에 함께 적용되게 합니다.

## 개인정보 보호

- **푸시를 켜거나 직접 테스트 메시지를 보내지 않는 한 확장 프로그램은 네트워크 요청을 하지 않습니다.** 텔레메트리도, 분석도, 원격 콘텐츠도 없습니다.
...
| `agentMonitor.push.events` | 모두 켜짐 | 푸시할 이벤트: `needsYou`, `error`, `limitHit`, `limitReset` |
...
푸시 설정은 사용자 설정에서만 읽으므로, 작업 영역 설정으로는 푸시를 켜거나 보낼 곳을 바꿀 수 없습니다.

...
- **聊天視窗的標題區只佔兩列**，面板矮也能直接看到智慧體。第一列是標題，**前往** 和 **壓縮…** 旁邊寫著上下文用了多少（「上下文 41%」）；第二列是狀態、上下文離自動壓縮還有多遠、提示、快取倒數和這個聊天的費用。其餘內容（上下文進度條、自動壓縮設定、今日合計、提示、續跑提示、記錄檔案那一列）收在 **詳細資料 ▸** 裡。遇到額度中斷、API 錯誤、壓縮迴圈這類緊急情況，才會多出一列。智慧體表的表頭在捲動時固定在頂端；面板矮於 400px 時，費用和用時兩欄會隱藏。
...
點擊聊天視窗標題區裡的 **前往**，或是按兩下清單裡的聊天視窗或它的某個智慧體，智慧體監視器就會叫出這個聊天視窗正在執行的地方。按一下仍然只是選取聊天視窗或顯示智慧體詳情。**前往對話** 也在聊天視窗的右鍵選單和 **…** 選單裡、側邊欄總覽的右鍵選單裡（聊天視窗那一列還有一個行內按鈕），以及命令選擇區裡。
...
- 如果你沒及時處理時還想在手機上收到訊息，見[推播到手機或團隊聊天](#推播到手機或團隊聊天)。想同時聽到提示音，見[提示音](#提示音)；不想夜裡被打擾，見[勿擾時段](#勿擾時段)。
...
- **收到門檻提醒**：用量、今天的費用或某個聊天的上下文超過了你設定的值（見[門檻提醒](#門檻提醒)）。對應的事件是 `usageHigh`、`costDaily` 和 `contextHigh`。

這些預設都開啟（費用和上下文提醒要等你設了門檻才會出現）。可以在設定選單的 **選擇推播事件…** 裡或用 `agentMonitor.push.events` 選擇。[勿擾時段](#勿擾時段)內不推播；如果你允許出錯照常提醒，API 錯誤和碰到額度上限仍會推播。

**連網：** 推播是唯一會連網的功能，所以還需要開啟 **允許連網**（`agentMonitor.network.allow`，預設關閉）。它關閉時什麼都不會傳出，連測試訊息也不會，設定選單裡推播會顯示為 **已暫停**。如果在它關閉時新增管道、開啟推播或傳送測試訊息，會先跳出一個對話框，說明會傳送什麼、傳到哪裡；只有按它的 **允許連網** 按鈕才會開啟。用 **禁止連網**（設定選單、面板的 **…** 選單或命令選擇區）可以再關掉：推播仍保持開啟，重新允許連網後，中間發生的事情不會補傳。允許連網只對這台電腦有效：設定同步不會把它帶到你的其他電腦上。

**傳送什麼：** 專案資料夾名稱和狀態，例如「智慧體需要你 · my-app」；額度相關的訊息則是產品名稱和重置時間；門檻提醒則說明超過了什麼：用量週期及其百分比、今天的費用超出了預算（不帶任何金額），或聊天視窗的上下文百分比。開啟 `agentMonitor.push.includeTitle` 後會加上對話標題（以及子智慧體名稱），但只限真正的標題：你自己設定的，或智慧體、應用程式產生的。從你第一則提示詞擷取的標題永遠不會傳送。擴充功能不會附加提示詞、程式碼、檔案路徑、token 數或費用；標題和子智慧體名稱按原樣傳送，裡面可能帶有檔案名稱。

**設定方法：** 從命令選擇區或面板標題列的 **…** 選單執行 **智慧體監視器：推播通知…**，選擇 **新增推播管道**。選一個服務，閱讀隱私說明（每個服務只顯示一次），填寫各欄位，再傳送一則測試訊息。權杖、金鑰和 Webhook 網址保存在 VS Code 的安全儲存空間裡，絕不會寫進 `settings.json`。編輯管道時，機密欄位留空就會保留已儲存的值；選填的機密（ntfy 存取權杖、飛書或釘釘的簽章密鑰）輸入 `-` 即可刪除。機密不會在電腦之間同步：開啟設定同步後，在另一台電腦上新增的管道會顯示 **這台電腦上尚未設定**，需要在這台電腦上再設定一次；移除管道時，只會刪掉目前這台電腦上的機密（留在其他電腦上的那份不會再被使用）。自架的 ntfy 或 Bark 伺服器只有本機和內網位址才能用 `http://`。它不加密，而且換到別的網路（咖啡廳、飯店）時，同一個位址可能是別人的裝置，所以只在你自己管理的網路裡使用。同一個選單會顯示推播是否開啟、有幾個管道在使用、是否允許連網，還能傳送測試訊息、編輯、關閉或移除管道、開關推播、選擇事件，以及允許或禁止連網。
...
選用功能，**預設關閉**。開啟 `agentMonitor.sound.enabled` 後，遇到下面這些情況，智慧體監視器會響一聲：

- **智慧體需要你**（`agentMonitor.sound.needsYou`）；
- **智慧體出錯停止**，例如 API 錯誤或碰到額度上限（`agentMonitor.sound.error`）；
...
選用功能，**預設關閉**。開啟 `agentMonitor.quietHours.enabled` 後，在設定的時段裡：

- 不響提示音；
- 不發送系統通知，訊息會等到你接下來切換到的 VS Code 視窗裡再顯示；
- 不推播到手機或團隊聊天，勿擾時段裡發生的事之後也不會補發。

前景 VS Code 視窗裡的訊息照常顯示，面板、狀態燈、計數標記和狀態列也照常更新。勿擾時段生效期間，狀態列的提示會註明。

- `agentMonitor.quietHours.start` 和 `agentMonitor.quietHours.end` 是本地時間，24 小時制 `HH:MM`，預設 22:00 和 08:00。結束時間早於開始時間時，勿擾時段會跨過午夜。兩者相同時，勿擾時段不會生效。
- `agentMonitor.quietHours.days` 可以讓它只在某幾天生效：`sun`、`mon`、`tue`、`wed`、`thu`、`fri`、`sat`；留空（預設）表示每天。以開始的那天為準：設成 `["fri"]` 時，週五 22:00 到週六 08:00 是勿擾時段。
- 開啟 `agentMonitor.quietHours.allowErrors` 後，API 錯誤和碰到額度上限時仍會響出錯的提示音，也仍會推播。額度重置和門檻提醒照樣保持安靜。

### 門檻提醒

某個數字超過你設定的值時，智慧體監視器也可以提醒你。提醒方式和「需要你處理」通知一樣（有 VS Code 視窗在前景時在視窗裡顯示訊息，否則發送系統通知）；開啟提示音時會響門檻提醒的聲音，開啟推播時還會推播。
...
- 每則提醒只由一個視窗顯示。
- 費用是依官網價目表估算的（見[用量與費用](#用量與費用)），「今天」依你電腦的本地時間計算。
- 開啟推播時，這些提醒也會推播（見[推播到手機或團隊聊天](#推播到手機或團隊聊天)）。費用提醒的推播只說今天的費用超出了預算，不帶任何金額。
...
上下文達到 20K token 以上的每個 Claude Code 或 Codex 聊天視窗都會出現一個 **壓縮…** 按鈕，聊天視窗清單裡滑鼠停留時的那一列上，以及它標題區（智慧體上方）裡都有。點擊之前什麼都不會執行。
...
- **百分比和視窗都直接使用 Claude Code 和 Codex 自己的數字。** 百分比用 Claude Code 自己的公式：上下文 token 數（最近一次呼叫的輸入 + 快取寫入 + 快取讀取）÷ 上下文視窗，四捨五入。視窗來自 Claude Code 這次工作階段的費用記錄（能看出模型是不是用 1M 視窗執行的），否則用模型的標準視窗；Codex 則來自 Codex 的日誌。其他工具見[支援的工具](#支援的工具)。
...
- **等值 API 費用：** 每個智慧體、每個聊天視窗，以及今日的總計（僅 Claude Code 和 Codex）。對於 Claude Code 聊天視窗，標題列顯示的是 Claude Code 自己的累計費用（「Claude Code 的統計」）；其餘的都是依 token 數與官網價目表估算出來的。訂閱方案不是這樣計費的，所以僅供比較參考。GitHub Copilot Chat 顯示的是 Copilot credits，而不是費用（見[支援的工具](#支援的工具)）。
...
- 這段期間的合計、每日平均，以及用了幾天；
- **依模型** 拆分：輸入、輸出、快取讀取、快取寫入、推理、全部 token 和估算費用。

![用量歷史頁面，資料是虛構的範例：近 30 天每天的估算費用（Claude Code 和 Codex 疊在一起）、合計，以及依模型的明細](../images/history.png)

說明：

- 在你的電腦上根據面板讀取的同一批記錄計算（見[它讀什麼](#它讀什麼)）。只有頁面開啟時才在背景讀取記錄，頁面顯示期間每分鐘更新一次。
- 結果快取在擴充功能的儲存空間裡，下次開啟只讀新增的部分。
- 費用和擴充功能其他地方一樣，是依官網價目表估算的等值 API 費用。沒有公開價格的模型不計入費用，頁面會註明這部分有多少 token。
- 日期依你電腦的本地時間劃分。
- 只統計還留在你電腦上的記錄。Claude Code 會在保留期（`cleanupPeriodDays`，預設 30 天）過後刪除舊記錄，所以更早的日子可能比你實際用的少。

### 額度與續跑

- 顯示 Claude Code 聊天視窗何時撞上工作階段、每週或模型額度，以及額度何時重置。
- 顯示 Codex 的 5 小時和每週用量百分比。
- 當一個 Claude Code 或 Codex 聊天視窗或智慧體因額度用完、API 錯誤或被中斷而停止時，**複製續跑提示…** 會複製一份現成的提示詞或終端機命令（`claude --resume …`、`codex resume …`）。它還會估算重新讀取上下文需要多少費用。
...
這個頁面只涵蓋 Claude Code 和 Codex。大小統計在背景進行，只在頁面開啟時才計算，最多每 10 分鐘一次。
...
同一套監視邏輯也能在終端機裡執行。它包含在原始碼儲存庫裡，不在擴充功能套件裡，需要 Node.js 20 或更高版本（在 Node.js 20、22 和 24 上測試過）：
...
只要 `agentMonitor.shareScanAcrossWindows` 是開啟的（預設如此），即使只開了一個視窗，各視窗也會在擴充功能的全域儲存空間裡共用一個資料夾（`shared-scan`）。裡面記著由哪個視窗讀取記錄，這個視窗還會把最新結果存到 `shared-scan/snapshot.json` 給其他視窗使用：聊天標題、專案資料夾、步驟和費用。結果一變這個檔案就會被覆寫，這個視窗正常關閉時會刪除它。每個視窗還會在這裡留一份自己的小記錄，寫著它的資料夾以及擴充主機和各個終端機的處理程序 ID，這樣[前往](#前往聊天視窗執行的地方)才能找到聊天在哪個視窗裡執行；請另一個視窗開啟某個聊天的請求也是這裡的一個小檔案，那個視窗接手時就會刪除它。通知方面，每次等待會在系統暫存目錄下的一個私人資料夾裡留一個小標記（只有雜湊值，沒有聊天內容），確保在所有編輯器裡只有一個視窗顯示這則通知。標記一天後刪除。開啟推播後，每個推播管道的傳送時間（不含訊息內容）也記在同一個資料夾裡（`push-sent.json`），這樣上限能在各視窗之間共同生效。

[用量歷史](#用量歷史)頁面會在擴充功能的儲存空間裡保留一份快取：它讀過的每個記錄檔案的路徑、大小和讀到的位置，每天每個模型的 token 數，以及用來避免重複計算的訊息 ID。不保存任何聊天內容。只有頁面開啟時，這份快取才會更新。

## 隱私

- **除非你開啟允許連網，擴充功能不會發出任何網路請求**（`agentMonitor.network.allow`，預設關閉）。關閉時，程式碼會拒絕每一個請求，推播通知和測試訊息也不例外。目前只有推播通知會用到它。沒有遙測、沒有分析、沒有遠端內容。
...
- **前往只在你使用時才查看處理程序。** 為了找到聊天在哪個終端機或視窗裡執行，它會列出你電腦上正在執行的處理程序（見[前往聊天視窗執行的地方](#前往聊天視窗執行的地方)）。這不會讀取你的聊天內容，也不會傳送到任何地方。只有在你確認之後，它才會控制 Terminal.app 或 iTerm2 去選取分頁，而且 macOS 也會先徵求你的許可。
...
- **提示音在本機播放**，用的是系統內建的播放程式（macOS 用 `afplay`，Linux 用 `canberra-gtk-play` 或 `paplay`，Windows 用 PowerShell）。不會透過網路傳送任何東西。
- **用量歷史在本機計算**，快取在擴充功能的儲存空間裡，不會傳送到任何地方。
- **你的對話內容留在本機。** 聊天標題、步驟和結果只會顯示在你自己的 VS Code 裡（通知則顯示在你自己電腦的通知區域）。
- **這些數字是給你看的，不是給模型看的。** 上下文大小、快取倒數和費用永遠不會傳給模型。

## 命令

所有命令都在 **智慧體監視器** 分類下，下表列出每個命令在哪裡能找到。對某個聊天視窗生效的命令可以從它的列執行；從命令選擇區執行時，會先問你選哪個聊天視窗（目前選取的排在最前面）。

| 命令 | 位置 | 作用 |
| --- | --- | --- |
| **顯示智慧體監視器** | 狀態列燈、命令選擇區 | 開啟智慧體監視器面板 |
| **在側邊欄顯示總覽** | 命令選擇區 | 在側邊欄開啟總覽樹狀檢視 |
| **重新整理** | 面板標題列、總覽標題列、命令選擇區 | 立即重新讀取記錄 |
| **開啟設定** | 面板標題列的 **…** 選單、總覽標題列、命令選擇區 | 開啟智慧體監視器的設定 |
| **顯示所有工作階段** / **只顯示目前工作區的工作階段** | 面板標題列、總覽標題列、命令選擇區 | 切換範圍 |
| **標記為已查看** / **全部標記為已查看** | 聊天視窗右鍵選單或 **…** / 面板標題列、命令選擇區 | 把「已完成·未查看」變成「已完成」 |
| **隱藏已完成的智慧體** / **顯示已完成的智慧體** | 面板標題列、總覽標題列、命令選擇區 | 隱藏或顯示已完成的智慧體 |
| **前往對話** | 聊天視窗標題區的 **前往**、按兩下聊天視窗或智慧體、聊天視窗右鍵選單或 **…**、總覽右鍵選單和聊天視窗列、命令選擇區 | 叫出聊天視窗執行的地方；見[前往聊天視窗執行的地方](#前往聊天視窗執行的地方) |
...
| **複製續跑提示…** | 聊天視窗右鍵選單或 **…**（Claude Code 或 Codex 聊天視窗有可續跑內容時） | 複製續跑提示詞或終端機命令 |
| **壓縮…** | 聊天視窗列、右鍵選單或 **…**（上下文達到 20K 或以上的 Claude Code 和 Codex 聊天視窗）、聊天視窗標題區、命令選擇區 | 壓縮這個聊天視窗；見[壓縮按鈕](#壓縮按鈕可選模型帶費用估算) |
| **寫交接筆記並開新工作階段…** | 聊天視窗右鍵選單或 **…**（Claude Code 和 Codex 聊天視窗）、壓縮選單、命令選擇區 | 讓模型寫一份 `HANDOFF.md`，然後引導你 `/clear` 並繼續 |
| **設定自動壓縮閾值…** | 聊天視窗詳細資料裡的「自動壓縮」、聊天視窗右鍵選單或 **…**（Claude Code 和 Codex 聊天視窗）、命令選擇區 | 見[自己設定自動壓縮容量](#自己設定自動壓縮容量) |
| **儲存位置與占用** | 面板標題列的 **…** 選單、命令選擇區 | 見[聊天視窗存在哪裡](#聊天視窗存在哪裡以及怎麼搬移) |
| **推播通知…** | 面板標題列的 **…** 選單、命令選擇區 | 見[推播到手機或團隊聊天](#推播到手機或團隊聊天) |
| **允許連網** / **禁止連網** | 面板標題列的 **…** 選單、命令選擇區 | 開啟或關閉連網，見[隱私](#隱私) |
| **切換連網** | 只能透過你在「鍵盤快速鍵」裡設定的快速鍵 | 連網關著就開啟，開著就關閉 |
| **檢視用量歷史** | 面板標題列的 **…** 選單、命令選擇區 | 見[用量歷史](#用量歷史) |
...
| `agentMonitor.sound.enabled` | `false` | 播放提示音，見[提示音](#提示音) |
| `agentMonitor.sound.needsYou` | `default` | 智慧體需要你時的聲音：`default`、`off`，或 `Glass`、`Ping`、`Pop`、`Tink`、`Submarine`、`Funk`、`Hero`、`Basso` |
| `agentMonitor.sound.error` | `default` | 智慧體出錯或碰到額度上限而停止時的聲音（選項同上） |
| `agentMonitor.sound.done` | `default` | 聊天視窗已完成而你還沒看時的聲音（選項同上） |
| `agentMonitor.sound.alert` | `default` | 門檻提醒的聲音（選項同上） |
| `agentMonitor.quietHours.enabled` | `false` | 勿擾時段：不響提示音、不發送系統通知、不推播，見[勿擾時段](#勿擾時段) |
| `agentMonitor.quietHours.start` | `"22:00"` | 勿擾時段的開始時間（本地時間，24 小時制 `HH:MM`） |
| `agentMonitor.quietHours.end` | `"08:00"` | 勿擾時段的結束時間；早於開始時間表示到隔天 |
| `agentMonitor.quietHours.days` | `[]` | 勿擾時段在哪幾天開始：`sun` … `sat`；留空表示每天 |
| `agentMonitor.quietHours.allowErrors` | `false` | 勿擾時段內，API 錯誤和碰到額度上限時仍照常響出錯的提示音並推播 |
| `agentMonitor.alerts.usagePercent` | `90` | Codex 的 5 小時或每週用量達到這個百分比時提醒（0 表示關閉），見[門檻提醒](#門檻提醒) |
| `agentMonitor.alerts.dailyCost` | `0` | 今天的估算費用達到這麼多美元時提醒（0 表示關閉） |
| `agentMonitor.alerts.contextPercent` | `0` | 聊天視窗的主對話達到自動壓縮點的這個百分比時提醒（0 表示關閉） |
| `agentMonitor.network.allow` | `false` | 允許網路請求。關閉時擴充功能不會發出任何網路請求；目前只有推播通知會用到它。只對這台電腦有效（設定同步不會同步它） |
| `agentMonitor.push.enabled` | `false` | 推播到手機或團隊聊天，見[推播到手機或團隊聊天](#推播到手機或團隊聊天) |
| `agentMonitor.push.events` | 全部開啟 | 哪些事件要推播：`needsYou`、`error`、`limitHit`、`limitReset`、`usageHigh`、`costDaily`、`contextHigh` |
...
推播相關的設定和 `agentMonitor.network.allow` 只從你的使用者設定裡讀取，所以工作區設定既不能開啟推播或連網，也不能改變推播的去向。

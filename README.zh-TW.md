...
![Agent Monitor 在 VS Code 面板中的動態示範，資料是虛構的範例：一個在終端機裡執行的 Claude Code 聊天視窗裡，主智慧體和兩個子智慧體依序讀取檔案、搜尋、編輯、執行測試，token、費用和上下文百分比隨之上升。一個子智慧體要執行測試時，它和主智慧體都亮起洋紅色燈等你核准，清單裡這個聊天的燈、面板分頁上的計數和狀態列也跟著改變。按兩下這個聊天，跳到它所在的終端機，在那裡核准命令。回到面板，所有智慧體完成，亮起綠燈。最後從面板的 … 選單開啟用量歷史](images/demo.gif)
...
- **一步回到它執行的地方。** 點 **前往** 或按兩下一個聊天，就能叫出它執行的地方：Claude Code 或 Codex 擴充功能、Copilot Chat 裡的那個聊天，或它所在的 VS Code 終端機，在另一個 VS Code 視窗裡也行（在 macOS 上還包括 Terminal.app 或 iTerm2 的分頁）。
...
- **出錯或碰到額度上限後接著做。** 顯示額度什麼時候重置，並準備好繼續用的提示詞或命令，複製就能用。也可以為某個專案開啟自動續跑：API 錯誤後，或額度重置後，智慧體監視器會在背景讓 Claude Code 聊天接著做，通知裡也會說明什麼時候繼續。
...
- 要在背景壓縮一個已關閉的聊天視窗，或使用自動續跑，還需要 Claude Code 命令列（自動續跑需要支援 `--bg` 的版本，已用 2.1.283 測試過）。擴充功能會先在 PATH 裡找 `claude`，再到已安裝的 Claude Code 擴充功能裡找。你也可以設定 `agentMonitor.claude.cliPath`。
...
- **前往** 只在你使用時才查看你電腦上正在執行的處理程序，用來找到聊天在哪裡執行。它不會為此讀取你的聊天內容，也不會傳出任何資料。
- 只有在你確認之後，它才會修改檔案或執行 Claude Code；自動續跑則只在你開啟它的專案裡執行（見[免責聲明](#免責聲明)）。
...
- **只在你確認後才動手。** 本擴充功能讀取你電腦上的工作階段記錄。只有在你確認之後，它才會修改檔案或執行 Claude Code：修改自動壓縮設定（只改一項，並留有備份）、在背景壓縮、寫交接筆記，或在背景繼續聊天。自動續跑執行 Claude Code 前不會每次都問你，但只在你開啟它的專案裡執行。
- **用量和費用由你承擔。** 背景壓縮、交接筆記和自動續跑執行的是你自己的 Claude Code，會計入你方案的用量額度或 API 帳單。顯示的費用是依官方價目表估算的，不是帳單。

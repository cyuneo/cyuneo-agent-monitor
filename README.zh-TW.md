...
> **預覽版（0.3.1）。** 這是首個公開版本。如果發現任何不對勁的地方，歡迎到 [GitHub Issues](https://github.com/cyuneo/cyuneo-agent-monitor/issues) 回報。

![Agent Monitor 在 VS Code 面板中，佈局和終端機一樣：選取聊天視窗的智慧體佔了面板的大部分，一列帶狀態燈的窄聊天清單放在終端機分頁清單所在的一側](images/split-view.png)
...
它只讀取 Claude Code 和 Codex 本來就寫在你電腦上的記錄；擴充功能本身不連網，不收集任何資料。
...
- **遵守各服務的條款。** 你需要自行確保對 Claude Code、Codex 及其服務的使用符合它們的條款。

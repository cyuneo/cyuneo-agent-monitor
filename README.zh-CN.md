...
![Agent Monitor 在 VS Code 面板中的动态演示，数据是虚构的示例：一个在终端里运行的 Claude Code 聊天窗口里，主智能体和两个子智能体依次读文件、搜索、编辑、跑测试，token、费用和上下文百分比随之上涨。一个子智能体要运行测试时，它和主智能体都亮起品红色灯等你批准，列表里这个聊天的灯、面板标签上的计数和状态栏也跟着变。双击这个聊天，跳到它所在的终端，在那里批准命令。回到面板，所有智能体完成，亮起绿灯。最后从面板的 … 菜单打开用量历史](images/demo.gif)
...
- **一步回到它运行的地方。** 点 **转到** 或双击一个聊天，就能调出它运行的地方：Claude Code 或 Codex 扩展、Copilot Chat 里的那个聊天，或它所在的 VS Code 终端，在另一个 VS Code 窗口里也行（在 macOS 上还包括 Terminal.app 或 iTerm2 的标签页）。
...
- **出错或撞额度后接着干。** 显示额度什么时候重置，并准备好继续用的提示词或命令，复制就能用。也可以为某个项目开启自动续跑：API 出错后，或额度重置后，智能体监视器会在后台继续停下的 Claude Code 聊天，通知里会告诉你什么时候继续。
...
- 要在后台压缩一个已关闭的聊天窗口，或者使用自动续跑，还需要 Claude Code 命令行（自动续跑需要支持 `--bg` 的版本，已在 2.1.283 上测试过）。扩展会先在 PATH 里找 `claude`，再到已安装的 Claude Code 扩展里找。你也可以设置 `agentMonitor.claude.cliPath`。
...
- **转到** 只在你使用时才查看你电脑上正在运行的进程，用来找到聊天在哪里运行。它不会为此读取你的聊天内容，也不会发出任何数据。
- 只有在你确认之后，它才会修改文件或运行 Claude Code；自动续跑则只在你开启了它的项目里运行 Claude Code（见[免责声明](#免责声明)）。
...
- **只在你确认后才动手。** 本扩展读取你电脑上的会话记录。只有在你确认之后，它才会修改文件或运行 Claude Code：修改自动压缩设置（只改一项，并留有备份）、在后台压缩、写交接笔记、在后台继续一个聊天。自动续跑运行 Claude Code 前不会每次都问你，但只在你开启了它的项目里运行。
- **用量和费用由你承担。** 后台压缩、交接笔记和自动续跑运行的是你自己的 Claude Code，会计入你套餐的用量额度或 API 账单。显示的费用是按官方价目表估算的，不是账单。

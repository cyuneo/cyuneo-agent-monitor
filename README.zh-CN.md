...
> **预览版（0.4.0）。** 发现任何不对劲的地方，欢迎到 [GitHub Issues](https://github.com/cyuneo/cyuneo-agent-monitor/issues) 反馈。
...
- **不在电脑前也能收到提醒。** 聊天开始等你时，VS Code 或系统会通知你；愿意的话，还可以把简短消息推送到手机或团队聊天（ntfy、Bark、Server酱、飞书、钉钉、企业微信、Telegram、Discord、Slack），出错和撞额度时也会推送。
- **把上下文管起来。** 看到每个聊天用了多满；在面板里直接压缩（可以换便宜的模型，先看费用估算）；自己决定什么时候自动压缩；缓存快过期前提醒你。
- **撞额度后接着干。** 显示额度什么时候重置，并准备好继续用的提示词或命令，复制就能用。
- **知道聊天记录存在哪。** 看到占了多少空间，并给出把它们搬到别的磁盘的参考命令。

它只读取 Claude Code 和 Codex 本来就写在你电脑上的记录，不收集任何数据；除非你开启推送通知（默认关闭）或自己发送测试消息，扩展本身不联网。
...
- 没有遥测；除非你开启推送通知或自己发送测试消息，扩展本身不联网。你的对话只留在你自己的电脑上。
...
- **遵守各服务的条款。** 你需要自行确保对 Claude Code、Codex 及其服务，以及你设置的推送服务的使用符合它们的条款。

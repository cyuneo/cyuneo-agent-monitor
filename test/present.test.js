...
// Sessions shaped like the Copilot provider produces them (lib/providers/copilot.js):
...
function lampTests() {
  ...
  test('Copilot feeds the same lamps; guesses are recognised', () => {
    ...
    assert.ok(lamp.isGuessStatus(st('done', NOW - 1000, { certainty: 'guess' })));
...
function syntheticSnapshot(now) {
  ...
  // Copilot sessions, including Copilot waiting parts and a Copilot session without credits
  sessions.push(
...
function formatTests() {
  ...
  test('left row: {Claude|Codex} · {status} · {context}, text does not change as time passes', () => {
    ...
    for (const s of snap.sessions) {
      ...
      assert.ok(/^(Claude|Codex|Copilot) · /.test(a.description), a.description);
    }
    const s = session({
  ...
  test('Copilot: product name, sub-agent kinds, waiting parts by name', () => {
    const en = i18nLib.createI18n('en', { timeZone: 'UTC' });
    for (const locale of LOCALES) {
      const i18n = i18nLib.createI18n(locale, { timeZone: 'UTC' });
      assert.deepStrictEqual(['copilot'].map((p) => fmt.providerLabel(p, i18n)), ['Copilot'], locale);
    }
    ...
    assert.strictEqual(fmt.formatAgentKind({ kind: 'copilotSubagent' }, en), 'Subagent');
  ...
  test('guessed statuses: approval guesses keep their own wording and note; live text', () => {
    const en = i18nLib.createI18n('en', { timeZone: 'UTC' });
  ...
  test('costs: Copilot shows credits (never "$0" or "No public price"); price date', () => {
    const en = i18nLib.createI18n('en', { timeZone: 'UTC' });
  ...
  test('no auto-compact point for Copilot: never "auto-compact is off", no compaction row, no compact menu; window source named', () => {
    const en = i18nLib.createI18n('en', { timeZone: 'UTC' });
    for (const s of [copilotSession()]) {
      const tk = fmt.sessionContextTokens(s);
  ...
  test('token counts never recorded (Copilot sub-agents, Copilot chats without usage): "—" instead of 0, no percentage, bar or row context', () => {
    ...
    assert.strictEqual(fmt.tokensUnknown(agent({ tokens: { ...zero, unknown: true } }), 'codex'), true, 'explicit flag');
  ...
  test('last usage-limit hits: Claude and Copilot each get a line; the panel keeps a hit without a reset time for an hour', () => {
    const en = i18nLib.createI18n('en', { timeZone: 'UTC' });
    ...
    assert.strictEqual(fmt.formatLastHit('copilot', hit({ ms: undefined }), en, NOW), 'Last Copilot limit hit: Usage limit reached');
    assert.strictEqual(fmt.formatLastHit('copilot', null, en, NOW), '');
    ...
    const lines = fmt.formatLastHits({
      claude: { lastHit: claude }, codex: { windows: [] }, copilot: { lastHit: hit({ ms: NOW - 2 * HOUR }) },
    }, en, NOW);
    assert.deepStrictEqual(lines.map((l) => l.split(':')[0]), ['Last Claude limit hit', 'Last Copilot limit hit']);
    ...
    for (const loc of LOCALES) {
      ...
      assert.ok(!PLACEHOLDER.test(fmt.formatLastHit('copilot', hit({ ms: null }), i18n, NOW)), loc);
    }
  });

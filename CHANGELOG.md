...
- **Three more tools: GitHub Copilot Chat, and Gemini CLI and Qwen Code as a preview.** Besides Claude Code and Codex, the panel now shows chats from **GitHub Copilot Chat** (VS Code's built-in chat, agent mode included), **Gemini CLI** and **Qwen Code**, with their status lights, agents, steps and tokens. Tools that aren't installed are skipped. Everything is read-only: for Copilot Chat, the chat session files VS Code keeps in its User folder (`workspaceStorage/*/chatSessions`, chats from windows with no folder open, and other profiles); for Gemini CLI, `~/.gemini/tmp/<project>/chats` (and the same under `~/.cache/.gemini` when it runs in the macOS sandbox); for Qwen Code, its chats folder and the `runtime.json` next to each chat. The extension never writes to them, and their contents are never sent anywhere. What each tool shows differs:
  ...
  - **Qwen Code (preview):** whether a chat is open comes from its process ID and is exact; "needs you" is a guess. Costs are estimated at Alibaba Cloud's international list prices; the free OAuth model shows tokens only.
  - None of the three has an auto-compact point. Usage history, today's total cost and the storage page still cover only Claude Code and Codex, and background compaction is still Claude Code only.
  ...
  - Gemini CLI and Qwen Code support is built from their published record formats and hasn't been checked against real sessions yet. Please report anything that looks wrong in [GitHub Issues](https://github.com/cyuneo/cyuneo-agent-monitor/issues).
- **Settings for the new tools:** `agentMonitor.copilot.enabled`, `agentMonitor.gemini.enabled` and `agentMonitor.qwen.enabled` (all on by default), and `agentMonitor.gemini.home` and `agentMonitor.qwen.home` for their folders. Left empty, Gemini CLI is read from `$GEMINI_CLI_HOME/.gemini` or `~/.gemini`, plus its macOS sandbox folder (`$GEMINI_CLI_HOME/.cache/.gemini` or `~/.cache/.gemini`), and Qwen Code from `$QWEN_RUNTIME_DIR`, else `$QWEN_HOME`, else `~/.qwen`.
- **The terminal version reads the new tools too.** `--provider` now takes `all` (the default), `claude`, `codex`, `copilot`, `gemini` or `qwen`.
